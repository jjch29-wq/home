...
class DraggableMessagebox:
    ...
    def _show(type, title, message):
        ...
        lbl_msg = tk.Label(main_frame, text=message, font=("Malgun Gothic", 10), justify="left", wraplength=300, background="#f3f4f6")
        lbl_msg.pack(side="left", fill="both", expand=True)

        btn_frame = tk.Frame(dialog, background="#f3f4f6", pady=15)
        btn_frame.pack(fill='x')
        
        btn_ok = ttk.Button(btn_frame, text="확인", command=close_dialog)
    ...
    def _select(self, val):
        def commit():
            try:
                ...
                self.hide()
                # Trigger events for auto-save
                self.widget.event_generate('<<ComboboxSelected>>')
                self.widget.event_generate('<Return>')
            except: pass
        
        # Small visual feedback delay
        self.widget.after(50, commit)
...
def register_autocomplete(combobox, suggestion_list):
    ...
    def on_keyrelease(event):
        ...
        if event.keysym == "Up" and combobox._suggestion_win.active:
            combobox._suggestion_win.move_selection(-1)
            return "break"
        if event.keysym == "Return" and combobox._suggestion_win.active:
            if combobox._suggestion_win.confirm_selection():
                return "break"
        
        if event.keysym in ("Left", "Right", "Escape", "Tab"):
            return
        ...
        combobox._autocomplete_timer = combobox.after(50, perform_filter)
    ...
    def on_focus_in(event):
        ...
        if not typed:
            if isinstance(suggestion_list, list):
                event.widget['values'] = suggestion_list
...
class ColumnSelectionDialog(tk.Toplevel):
    ...
    def __init__(self, parent, columns, title="엑셀 출력 컬럼 선택"):
        ...
        for col in columns:
            ...
            self.vars[col] = var
            # [NEW] Map 필름매수 to RT매수 and 검사량 to 수량 for display
            display_map = {'필름매수': 'RT매수', '검사량': '수량'}
...
class MaterialManager:
        ...
        if hasattr(self, 'entry_canvas') and self.entry_canvas:
            try:
                ...
                if bbox:
                    ...
                    self.entry_canvas.configure(scrollregion=(0, 0, bbox[2], bbox[3] + 50))
                else:
                ...
                try:
                    ...
                    if not self.daily_usage_df.empty:
                        ...
                        for col in string_columns:
                            ...
                            self.daily_usage_df[col] = self.daily_usage_df[col].astype(str).replace(['nan', 'None', 'NULL', '-0.0', '0.0', 'NaN', 'NAN', 'nan.0'], '')
    ...
    def enable_autocomplete(self, combobox, values_list_attr=None, values_list=None):
        ...
        def perform_filter(event_widget):
            typed = event_widget.get()
            ...
            if not typed:
                ...
                # [NEW] Show full list on empty input if focused
        ...
        def on_keyrelease(event):
            ...
            if event.keysym in ("Left", "Right", "Up", "Down", "Return", "Tab", "Shift_L", "Shift_R", "Control_L", "Control_R"):
                if event.keysym == "Tab":
                    suggest_win.hide()
                return
            ...
            timer_id = self.root.after(50, lambda: perform_filter(widget))
        ...
        combobox.bind('<KeyPress-Down>', handle_extra_nav)
        combobox.bind('<KeyPress-Up>', handle_extra_nav)
        combobox.bind('<KeyPress-Return>', handle_extra_nav)
        combobox.bind('<KeyPress-Escape>', lambda e: suggest_win.hide())
        combobox.bind('<KeyPress-Tab>', lambda e: suggest_win.hide())
        ...
        def on_focus_in(event):
            ...
            # [NEW] Trigger filter immediately on focus to show current history/list
            perform_filter(event.widget)
    ...
    def create_widgets(self):
        ...
        self.notebook.add(self.tab_reports, text='년별 집계')
    ...
    def setup_report_tab(self):
        ...
        display_frame = ttk.LabelFrame(self.tab_reports, text="년별 집계 (현장별 데이터 자동 집계)")
        ...
        # Treeview with columns including worker, work time, and OT fields (Matching Monthly Aggregation)
        ...
        # Column configuration match Monthly Aggregation
        col_widths = {
        ...
        for col in columns:
            ...
            self.report_tree.heading(col, text=col, command=lambda c=col: self.treeview_sort_column(self.report_tree, c, False))
        ...
        self.report_tree.bind('<ButtonRelease-1>', lambda e: self.save_tab_config())
    ...
    def update_report_view(self):
        ...
        for _, entry in grouped.iterrows():
            ...
            # Consolidate worker names, work times, and OTs
            def clean_str(val):
            ...
            for col in worker_cols:
                if col in entry and entry[col]:
                    all_workers.extend([w.strip() for w in entry[col].split(',') if w.strip()])
            worker_str = self.format_worker_summary(all_workers)
            
            all_worktimes = set()
            for col in worktime_cols:
                if col in entry and entry[col]:
                    all_worktimes.update([t.strip() for t in entry[col].split(',') if t.strip()])
            worktime_str = ", ".join(sorted(all_worktimes))
            
            # Collect OTs individually
            ot_values = []
            for i in range(1, 11):
                col_name = 'OT' if i == 1 else f'OT{i}'
                val = entry.get(col_name, '')
                ot_values.append(val)
                if val:
                    try:
                        # Extract amount if in (X원) format, else try parse
                        if '(' in val and '원' in val:
                            a_str = val.split('(')[1].split('원')[0].replace(',', '')
                            total_indiv_ot_amounts[i-1] += int(float(a_str))
                        else:
                            total_indiv_ot_amounts[i-1] += self.calculate_ot_amount(val)
                    except: pass
            
            if clean_str(entry.get('Note', '')): has_note = True
            
            # Get aggregated values
            film_count = entry.get('FilmCount', 0.0) # Changed from 'Film Count'
            ...
            # Insert row with new column order
            values = (
                entry['Year'], entry['Month'], entry['Site'], 
                worker_str, worktime_str, *ot_values,  # New columns
                f"{test_amount:g}", f"{unit_price:g}", f"{travel_cost:g}", f"{meal_cost:g}", f"{test_fee:g}", 
                mat_name, f"{film_count:g}",
                f"{rtk_center:g}", f"{rtk_density:g}", f"{rtk_marking:g}", f"{rtk_film:g}", 
                f"{rtk_handling:g}", f"{rtk_customer:g}", f"{rtk_other:g}", f"{rtk_total:g}",
                f"{ndt_fluorescent_mag:g}", f"{ndt_magnet:g}", f"{ndt_paint:g}", f"{ndt_penetrant:g}", 
                f"{ndt_cleaner:g}", f"{ndt_developer:g}", f"{ndt_fluorescent_pen:g}", 
                ""
            )
        ...
        if not grouped.empty:
            ...
            # Create empty placeholders for OT columns
            empty_ots = [''] * 10
            ...
            values = (
                ...
                '', # 작업시간
                *empty_ots, # OT1~10
                f"{total_test_amount:g}" if total_test_amount > 0 else '',
                ...
                '', # 품목명
                f"{total_film_count:g}" if total_film_count > 0 else '',
                f"{total_rtk_center:g}" if total_rtk_center > 0 else '',
                f"{total_rtk_density:g}" if total_rtk_density > 0 else '',
                f"{total_rtk_marking:g}" if total_rtk_marking > 0 else '',
                f"{total_rtk_film:g}" if total_rtk_film > 0 else '',
                f"{total_rtk_handling:g}" if total_rtk_handling > 0 else '',
                f"{total_rtk_customer:g}" if total_rtk_customer > 0 else '',
                f"{total_rtk_other:g}" if total_rtk_other > 0 else '',
                f"{total_rtk_sum:g}" if total_rtk_sum > 0 else '',
                f"{total_ndt_fluorescent_mag:g}" if total_ndt_fluorescent_mag > 0 else '',
                f"{total_ndt_magnet:g}" if total_ndt_magnet > 0 else '',
                f"{total_ndt_paint:g}" if total_ndt_paint > 0 else '',
                f"{total_ndt_penetrant:g}" if total_ndt_penetrant > 0 else '',
                f"{total_ndt_cleaner:g}" if total_ndt_cleaner > 0 else '',
                f"{total_ndt_developer:g}" if total_ndt_developer > 0 else '',
                f"{total_ndt_fluorescent_pen:g}" if total_ndt_fluorescent_pen > 0 else '',
                ''
            )
            self.report_tree.insert('', 'end', values=values, tags=('total',))
            ...
            all_cols = self.report_tree['columns']
            visible_cols = [col for col in all_cols if col in mandatory_cols or dynamic_col_status.get(col, False)]
            self.report_tree['displaycolumns'] = visible_cols
            
            # Ensure stretch=False and minwidth
            for col in visible_cols:
                self.report_tree.column(col, stretch=False, minwidth=20)
            
            # Re-apply saved widths
            if hasattr(self, 'tab_config') and 'report_usage_col_widths' in self.tab_config:
                saved_widths = self.tab_config['report_usage_col_widths']
                for col in visible_cols:
                    if col in saved_widths:
                        try: self.report_tree.column(col, width=int(saved_widths[col]))
                        except: pass
            
            # Ensure Total Row stays at bottom
            self.report_tree.detach(self.report_tree.get_children()[-1])
            self.report_tree.insert('', 'end', values=values, tags=('total',))
        else:
            # If empty
        ...
        try:
            ...
            if tree == getattr(self, 'daily_usage_tree', None):
                try:
                    ...
                    if f_count and float(f_count) > 0: full_info.append(f"• RT매수: {f_count}")
            ...
            if final_x + popup_width > screen_w:
                final_x = screen_w - popup_width - 10
            if final_x < 10:
                final_x = 10
                
            # Check Bottom boundary - if it hides below, show ABOVE the cell
            if final_y + popup_height > screen_h:
                final_y = root_y - popup_height - 1
                # If even showing above goes off screen top (unlikely but safe), stick to top
                if final_y < 10: final_y = 10
            ...
            # Don't close immediately on click inside, allow reading
            # But let's add a close button or close on double-click
            popup.bind("<Double-1>", close_popup)
            
            # Close on any click outside
            self.root.bind("<Button-1>", lambda e: close_popup() if popup.winfo_exists() and e.widget != popup and e.widget != lb else None, add="+")
    ...
    def setup_monthly_usage_tab(self):
        ...
        columns = ('연도', '월', '현장', '작업자', '작업시간', 'OT시간', 'OT금액', 'OT1', 'OT2', 'OT3', 'OT4', 'OT5', 'OT6', 'OT7', 'OT8', 'OT9', 'OT10', 
                   ...
                   '취급부주의', '고객불만', '기타', 'RT총계', '형광자분', '흑색자분', '백색페인트', '침투제', '세척제', '현상제', '형광침투제', '비고', '(Full작업자)')
        ...
        # 2. Bottom pane: Site and Worker Summaries Side-by-Side
        bottom_summary_frame = ttk.Frame(self.monthly_paned)
        self.monthly_paned.add(bottom_summary_frame, weight=1)
        
        # Horizontal paned for Site/Worker summaries
        h_paned = ttk.PanedWindow(bottom_summary_frame, orient="horizontal")
        h_paned.pack(expand=True, fill='both')
        
        self.monthly_usage_tree.bind("<Button-1>", lambda e: self.show_worker_popup(e, self.monthly_usage_tree), add="+")
        
        # -- Site Summary --
        site_frame = ttk.LabelFrame(h_paned, text="현장별 누계")
        h_paned.add(site_frame, weight=1)
        
        site_cols = ('현장', '총공수', 'OT시간', 'OT금액', '검사비', '출장비', '일식', '총합계')
        ...
        for col in site_cols:
            ...
            # Disable stretching and set fixed anchor
            self.site_summary_tree.column(col, width=80, anchor='center', stretch=False)
        ...
        # -- Worker Summary --
        worker_frame = ttk.LabelFrame(h_paned, text="작업자별 누계")
        h_paned.add(worker_frame, weight=1)
        
        worker_cols = ('작업자', '총공수', '주간', '야간', '휴일', 'OT시간', '주간OT', '야간OT', '휴일OT', 'OT합계')
        self.worker_summary_tree = ttk.Treeview(worker_frame, columns=worker_cols, show='headings')
        
        # Set column widths for worker summary
        worker_widths = {
            '작업자': 100, '총공수': 70, '주간': 60, '야간': 60, '휴일': 60, 
            'OT시간': 80, '주간OT': 90, '야간OT': 90, '휴일OT': 90, 'OT합계': 100
        }
        for col in worker_cols:
    ...
    def update_monthly_usage_view(self):
        ...
        self.report_tree.bind("<Button-1>", lambda e: self.show_worker_popup(e, self.report_tree), add="+")
    ...
    def _populate_monthly_summary_trees(self, df, has_note=None):
        ...
        for _, row in site_summary.iterrows():
            total_cost = row['검사비'] + row['출장비'] + row['일식']
            self.site_summary_tree.insert('', tk.END, values=(
                row['Site'],
                f"{row['WorkerCount']:.1f}",
                f"{row['OT시간']:.1f}",
                f"{row['OT금액']:,.0f}",
                f"{row['검사비']:,.0f}",
                f"{row['출장비']:,.0f}",
            ...
            if not worker_df.empty:
                ...
                def get_split_robust(row):
                    ...
                    if '시간' in ot_val or not (ot_val.replace(',', '').isdigit() and int(ot_val.replace(',', '')) > 100):
                        return self._calculate_split_ot_hours(ot_val)
                    else:
                        ...
                        return self._calculate_split_ot_hours(fake_ot_str)

                worker_df[['OT_H_Day', 'OT_H_Night']] = worker_df.apply(
                ...
                worker_df['H_Holiday'] = worker_df.apply(lambda r: r['OT_H'] if r['Shift_Holiday'] > 0 else 0.0, axis=1)
                worker_df['A_Holiday'] = worker_df.apply(lambda r: r['OT_A'] if r['Shift_Holiday'] > 0 else 0.0, axis=1)
        ...
        try:
            ...
            if save_path:
                ...
                for row in monthly_data:
                    new_row = list(row)
                    # Swap summarized worker with full list if (Full작업자) exists and row is long enough
                ...
                monthly_df = pd.DataFrame(final_data, columns=columns_to_export)
    ...
    def remove_box(self, key):
        ...
        is_dynamic = key.startswith('memo_') or key.startswith('clone_') or key.startswith('checklist_')
    ...
    def setup_daily_usage_tab(self):
        ...
        def _on_entry_config(e):
            # Update canvas window width as canvas changes
            self.entry_canvas.itemconfig(self.entry_canvas_window, width=e.width)
            self._ensure_canvas_scroll_region()
        
        self.entry_canvas.bind("<Configure>", _on_entry_config)
        
        # [REQUEST] Disable mouse scrolling
        ...
        form_container = ttk.Frame(self.left_pane, relief="solid", borderwidth=1)
        form_hdr = ttk.Frame(form_container)
        form_hdr.pack(side='top', fill='x', padx=0, pady=0)
        
        lbl_drag = ttk.Label(form_hdr, text="✥", font=('Arial', 9), cursor='fleur')
        lbl_drag.pack(side='left', padx=1)
        self.make_header_draggable(lbl_drag, form_container)
        
        lbl_title = ttk.Label(form_hdr, text="현장 정보 및 기록", font=('Malgun Gothic', 9, 'bold'))
        lbl_title.pack(side='left', padx=1)
        self.make_header_draggable(lbl_title, form_container)
        
        form_btn_box = ttk.Frame(form_hdr)
        form_btn_box.pack(side='right', padx=1)
        
        # Tools: Site Management icon
        btn_site_mgt = ttk.Label(form_btn_box, text="⚙️", font=('Arial', 8), cursor='hand2')
        btn_site_mgt.pack(side='left', padx=1)
        btn_site_mgt.bind('<Button-1>', lambda e: self.open_list_management_dialog('sites'))
        
        # Equip Management icon
        btn_equip_mgt = ttk.Label(form_btn_box, text="🛠️", font=('Arial', 8), cursor='hand2')
        btn_equip_mgt.pack(side='left', padx=1)
        btn_equip_mgt.bind('<Button-1>', lambda e: self.open_list_management_dialog('equipments'))

        # Delete icon
        btn_del = ttk.Label(form_btn_box, text="❌", font=('Arial', 8), cursor='hand2')
        btn_del.pack(side='left', padx=1)
        btn_del.bind('<Button-1>', lambda e: self.remove_box('form_box_geometry'))
        
        form_content = ttk.Frame(form_container, padding=(1, 2))
        form_content.pack(side='top', fill='both', expand=True)
        
        # Row 0: Date and Site
        ttk.Label(form_content, text="날짜:").grid(row=0, column=0, padx=2, pady=1, sticky='e')
        self.ent_daily_date = DateEntry(form_content, width=11, date_pattern='yyyy-mm-dd', locale='ko_KR')
        self.ent_daily_date.grid(row=0, column=1, padx=2, pady=1, sticky='w')
        self.ent_daily_date.set_date(datetime.datetime.now())
        
        ttk.Label(form_content, text="현장:").grid(row=0, column=2, padx=2, pady=1, sticky='e')
        self.ent_daily_site = ttk.Combobox(form_content, width=15, values=self.sites)
        self.ent_daily_site.grid(row=0, column=3, columnspan=3, padx=2, pady=1, sticky='ew')
        
        # [NEW] Focus Transition for Site
        def on_site_select(e):
            self.cb_daily_equip.focus_set()
        self.ent_daily_site.bind('<<ComboboxSelected>>', on_site_select)
        self.ent_daily_site.bind('<Return>', on_site_select)
        
        # Row 1: Equipment and Material
        ttk.Label(form_content, text="장비:").grid(row=1, column=0, padx=2, pady=1, sticky='e')
        self.cb_daily_equip = ttk.Combobox(form_content, width=12, values=getattr(self, 'equipments', []))
        self.cb_daily_equip.grid(row=1, column=1, padx=2, pady=1, sticky='w')

        # [NEW] Focus Transition for Equipment
        def on_equip_select(e):
            self.cb_daily_material.focus_set()
        self.cb_daily_equip.bind('<<ComboboxSelected>>', on_equip_select)
        self.cb_daily_equip.bind('<Return>', on_equip_select)
        
        ttk.Label(form_content, text="품목:").grid(row=1, column=2, padx=2, pady=1, sticky='e')
        self.cb_daily_material = ttk.Combobox(form_content, width=25)
        self.cb_daily_material.grid(row=1, column=3, columnspan=3, padx=2, pady=1, sticky='ew')
        
        # [NEW] Focus Transition for Material
        def on_mat_select(e):
            self.cb_daily_test_method.focus_set()
        self.cb_daily_material.bind('<<ComboboxSelected>>', on_mat_select)
        self.cb_daily_material.bind('<Return>', on_mat_select)
        
        # Row 2: Combined Row for Methods & Costs
        cost_row = ttk.Frame(form_content)
        cost_row.grid(row=2, column=0, columnspan=6, sticky='ew', pady=2)
        
        ttk.Label(cost_row, text="방법:").pack(side='left', padx=1)
        self.cb_daily_test_method = ttk.Combobox(cost_row, width=6, values=["RT", "PAUT", "UT", "MT", "PT", "PMI"])
        self.cb_daily_test_method.pack(side='left', padx=1)
        
        ttk.Label(cost_row, text="수량:").pack(side='left', padx=(5, 1))
        self.ent_daily_test_amount = ttk.Entry(cost_row, width=6)
        self.ent_daily_test_amount.pack(side='left', padx=1)
        
        ttk.Label(cost_row, text="단가:").pack(side='left', padx=(5, 1))
        self.ent_daily_unit_price = ttk.Entry(cost_row, width=8)
        self.ent_daily_unit_price.pack(side='left', padx=1)
        
        ttk.Label(cost_row, text="출장:").pack(side='left', padx=(5, 1))
        self.ent_daily_travel_cost = ttk.Entry(cost_row, width=8)
        self.ent_daily_travel_cost.pack(side='left', padx=1)
        
        ttk.Label(cost_row, text="일식:").pack(side='left', padx=(5, 1))
        self.ent_daily_meal_cost = ttk.Entry(cost_row, width=8)
        self.ent_daily_meal_cost.pack(side='left', padx=1)
        
        ttk.Label(cost_row, text="검사비:").pack(side='left', padx=(5, 1))
        self.ent_daily_test_fee = ttk.Entry(cost_row, width=8)
        self.ent_daily_test_fee.pack(side='left', padx=1)
        
        ttk.Label(cost_row, text="필름:").pack(side='left', padx=(5, 1))
        self.ent_film_count = ttk.Entry(cost_row, width=4)
        self.ent_film_count.pack(side='left', padx=1)
        
        # Add Sync button at the end of cost row for better flow
        btn_sync = ttk.Button(cost_row, text="일괄", command=self.sync_worker_times, width=4)
        btn_sync.pack(side='left', padx=(10, 1))
        
        form_container.grid(row=0, column=0, rowspan=3, columnspan=7, padx=1, pady=1, sticky='nw')
        ...
        def setup_worker_group(idx, row, col):
            ...
            # Store the entire group for easier access to new combined logic
        ...
        self.ent_daily_date.bind('<<DateEntrySelected>>', on_date_change)
        ...
        self.ent_daily_start_date = DateEntry(filter_frame, width=12, date_pattern='yyyy-mm-dd', locale='ko_KR')
        ...
        self.ent_daily_end_date = DateEntry(filter_frame, width=12, date_pattern='yyyy-mm-dd', locale='ko_KR')
        ...
        register_autocomplete(self.cb_daily_filter_site, None)
        ...
        register_autocomplete(self.cb_daily_filter_material, None)
        ...
        register_autocomplete(self.cb_daily_filter_equipment, None)
        ...
        register_autocomplete(self.cb_daily_filter_worker, None)
        
        ttk.Label(filter_frame, text="분류:").pack(side='left', padx=5)
        self.cb_daily_filter_shift = ttk.Combobox(filter_frame, width=10, state="readonly", values=["전체", "주간", "야간", "주야간", "휴일"])
        self.cb_daily_filter_shift.pack(side='left', padx=5)
        ...
        columns = ('날짜', '현장', '작업자', '작업시간', 'OT1', 'OT2', 'OT3', 'OT4', 'OT5', 'OT6', 'OT7', 'OT8', 'OT9', 'OT10', '장비명', '검사방법', '수량', '필름매수', '단가', '출장비', '일식', '검사비', 'OT시간', 'OT금액', '품목명', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RT총계', '형광자분', '흑색자분', '백색페인트', '침투제', '세척제', '현상제', '형광침투제', '비고', '입력시간', '(Full작업자)')
        ...
        visible_defaults = ['날짜', '현장', '작업자', '작업시간', '장비명', '검사방법', '수량', '필름매수', '단가', '출장비', '일식', '검사비', 'OT금액', '품목명', 'RT총계', '비고', '입력시간']
    ...
    def refresh_inquiry_filters(self):
        """Unified method to populate inquiry filter dropdowns for stability"""
        import re
        marker_pattern = MARKER_PATTERN
        
        # Site dropdown
        if hasattr(self, 'cb_daily_filter_site'):
            raw_sites = self.daily_usage_df['Site'].dropna().astype(str).str.strip().unique()
            unique_sites = ['전체'] + sorted([s for s in raw_sites if s])
        ...
        if hasattr(self, 'cb_daily_filter_material'):
            if 'MaterialID' in self.daily_usage_df.columns:
                ...
                self.cb_daily_filter_material['values'] = ['전체']
            if not self.cb_daily_filter_material.get():
                self.cb_daily_filter_material.set('전체')

        # Equipment dropdown
        if hasattr(self, 'cb_daily_filter_equipment'):
            raw_equip = self.daily_usage_df['장비명'].dropna().astype(str).str.strip().unique()
            unique_equip = ['전체'] + sorted([e for e in raw_equip if e])
            self.cb_daily_filter_equipment['values'] = unique_equip
            if not self.cb_daily_filter_equipment.get():
                self.cb_daily_filter_equipment.set('전체')
        ...
        if hasattr(self, 'cb_daily_filter_worker'):
            ...
            for w in all_workers_raw:
                w_str = str(w).strip()
                if not w_str or w_str.lower() in ['nan', '0.0']: continue
                cleaned = marker_pattern.sub('', w_str).strip()
                cleaned = " ".join(cleaned.split())
                if cleaned: clean_workers_set.add(cleaned)

            unique_workers = ['전체'] + sorted(list(clean_workers_set))
            self.cb_daily_filter_worker['values'] = unique_workers
            if not self.cb_daily_filter_worker.get():
                self.cb_daily_filter_worker.set('전체')

        # Site monthly (Aggregation tab)
        if hasattr(self, 'cb_filter_site_monthly'):
            raw_sites = self.daily_usage_df['Site'].dropna().astype(str).str.strip().unique()
            self.cb_filter_site_monthly['values'] = ['전체'] + sorted([s for s in raw_sites if s])
            
        # Material monthly (Aggregation tab)
        if hasattr(self, 'cb_filter_material_monthly'):
            m_id_col = 'MaterialID'
            if m_id_col in self.daily_usage_df.columns:
                unique_mat_ids = self.daily_usage_df[m_id_col].dropna().unique()
                material_names = []
                for mat_id in unique_mat_ids:
                    mat_row = self.materials_df[self.materials_df['MaterialID'] == mat_id]
                    if not mat_row.empty:
                        material_names.append(mat_row.iloc[0]['품목명'])
                self.cb_filter_material_monthly['values'] = ['전체'] + sorted(set(material_names))
    ...
    def update_daily_usage_view(self):
        ...
        if filter_site != '전체':
            ...
            filtered_df = filtered_df[filtered_df['Site'].astype(str).str.contains(filter_site, na=False, case=False)]
        
        if filter_equipment != '전체':
            # Support partial match for equipment
            filtered_df = filtered_df[filtered_df['장비명'].astype(str).str.contains(filter_equipment, na=False, case=False)]
        ...
        if not filtered_df.empty:
            ...
            # If user has manually selected columns, use those but still hide 0-value ones if possible
            if hasattr(self, 'manual_visible_cols') and self.manual_visible_cols:
                ...
                # Filter out columns that are all-zero in the current filtered set (unless mandatory)
                visible_cols = [col for col in visible_cols if col in mandatory_cols or dynamic_col_status.get(col, True)]
                
                # CRITICAL: Always ensure mandatory columns are included even if they were manually hidden
            ...
            # [NEW] Rename Heading '필름매수' to 'RT매수' for consistency (Main Inquiry View)
            try:
                self.daily_usage_tree.heading('필름매수', text='RT매수')
            except: pass
            ...
            if hasattr(self, 'manual_visible_cols') and self.manual_visible_cols:
                ...
                visible_cols = ['날짜', '현장', '작업자', '작업시간', '품목명', '입력시간']
                self.daily_usage_tree['displaycolumns'] = visible_cols
    ...
    def open_edit_daily_usage_dialog(self):
        ...
        ent_date = DateEntry(basic_frame, width=12, background='darkblue', foreground='white', borderwidth=2, date_pattern='yyyy-mm-dd')
    ...
    def export_daily_usage_history(self):
        ...
        try:
            ...
            if save_path:
                ...
                # [NEW] Hide columns that have no values (all empty, null, or 0/NaN depending on business logic)
                # But here we focus on completely empty/unused columns in the exported set
                cols_to_keep = []
                for col in df.columns:
                    # Check if the column has any non-empty, non-null value
                    # We use a helper check to see if there's anything "meaningful"
                    if not df[col].astype(str).str.strip().replace(['', 'nan', 'None', '0', '0.0'], np.nan).dropna().empty:
    ...
    def export_all_daily_usage(self):
        ...
        try:
            ...
            if save_path:
                ...
                # [NEW] Hide empty columns
                cols_to_keep = []
                for col in export_df.columns:
                    if not export_df[col].astype(str).str.strip().replace(['', 'nan', 'None', '0', '0.0'], np.nan).dropna().empty:
                        cols_to_keep.append(col)
    ...
    def _calculate_split_ot_hours(self, ot_value):
        """Split OT hours into Day window (18-22) and Night window (22+)"""
        import re
        try:
            if not ot_value or not str(ot_value).strip(): return 0.0, 0.0
            val = str(ot_value).strip().replace(' ', '')
            ...
            if range_match:
                ...
                # Note: we don't need minute precision for the split logic if we use the total_hours
            ...
            while remaining > 0:
                # Normalizing time to 24+ for next day
                # We care about 18:00-22:00 vs 22:00+
                if 18 <= current_time < 22:
                    ...
                    # Anything before 18 (unlikely for OT entry) or after 22
                    if current_time < 18:
                        can_take = 18 - current_time
                        taken = min(remaining, can_take)
                        # We'll count pre-18 as Night/Other for this specific requirement? 
                        # Actually user requested 18-22 is cumulative monthly OT. 
                        # Let's count pre-18 as 0 for "주간 OT" (Day) if it somehow happens.
                        night_hours += taken 
                        remaining -= taken
                        current_time += taken
                    else: # current_time >= 22
                        # Take until next day 18:00 (end of simulation essentially)
                        can_take = (18 + 24) - current_time 
                        taken = min(remaining, can_take)
                        night_hours += taken
                        remaining -= taken
                        current_time += taken
