...
PAGE_NUMBER_CELL = "P2" 
GRADE_LIST_STRING = "A240 304,A312 TP304,A312 F304,A403 WP304,A479 304,SS304,SS316,A182 F316,A312 TP316,A240 S32205,A928 S32205,A790 S32205,A182 F60,A479 S32205,A815 S32205"
...
def open_position_settings_ui():
    ...
    def on_apply():
        ...
        try:
            ...
            # Data Parsing (SITCO Y값 등 확실히 업데이트)
...
def place_image_freely(ws, img_path, anchor_cell_str, w, h, x_offset, y_offset):
    try:
        ...
        # [Debug]
        print(f"   [Logo] {os.path.basename(img_path)} -> {anchor_cell_str} (X:{x_offset}, Y:{y_offset}, W:{w}, H:{h})")
...
def add_logos_to_sheet(ws, is_cover=False):
    ...
    path_left = find_image_smart("PMI-1"); 
    ...
    try:
        ...
        if not isinstance(cell, MergedCell): cell.value = value
            ...
            for r in ws.merged_cells.ranges:
                if cell.coordinate in r: ws.cell(row=r.min_row, column=r.min_col).value = value; return
    except: pass
...
def force_print_settings(source_ws, target_ws):
    try:
        target_ws.print_area = f'A1:T{PRINT_END_ROW}'
...
def fill_excel_pagination(wb, data_list):
    ...
    current_page_num = 1; ws = wb.worksheets[current_sheet_idx]
    ws.title = f"{ws.title[:20]}_{current_page_num:03d}"
    try: ws[PAGE_NUMBER_CELL] = f"{current_page_num:03d}"
    except: pass 
    
    # ★ 갑지(Cover) 모드 적용 (첫번째 시트)
    is_cover = (current_sheet_idx == 0)
    add_logos_to_sheet(ws, is_cover=is_cover)
    force_print_settings(ws, ws)
    
    header_row = 17; col_map = {'No': 7} 
    for c in range(1, ws.max_column + 1):
        val = str(ws.cell(row=header_row, column=c).value).upper().strip()
        if "CR" in val: col_map['Cr'] = c
        elif "NI" in val: col_map['Ni'] = c
        elif "MO" in val: col_map['Mo'] = c
        elif "MN" in val: col_map['Mn'] = c

    current_row = START_ROW 
    for i, data in enumerate(data_list):
        if current_row > DATA_END_ROW:
            current_page_num += 1; ws = prepare_next_sheet(wb, current_sheet_idx, current_page_num)
            try: ws[PAGE_NUMBER_CELL] = f"{current_page_num:03d}"
            except: pass
            current_sheet_idx += 1; current_row = START_ROW 
        
        write_safe(ws, current_row, col_map['No'], data['No'])
        if 'Cr' in col_map: write_safe(ws, current_row, col_map['Cr'], data['Cr'])
        if 'Ni' in col_map: write_safe(ws, current_row, col_map['Ni'], data['Ni'])
        if 'Mo' in col_map: write_safe(ws, current_row, col_map['Mo'], data['Mo'])
        if 'Mn' in col_map: write_safe(ws, current_row, col_map['Mn'], data['Mn'])
        write_safe(ws, current_row, 14, data['Grade'])
        current_row += 1
    print(f"   ✅ 데이터 입력 완료")
