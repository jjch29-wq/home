...
class DraggableMessagebox:
    ...
    def _show(type, title, message):
        ...
        lbl_msg = tk.Label(main_frame, text=message, font=("Malgun Gothic", 10), justify="left", wraplength=320, background="#f3f4f6")
...
class VehicleInspectionWidget(ttk.Frame):
    ...
    def __init__(self, parent, theme_bg='#f0f0f0', vehicle_list=None, **kwargs):
        ...
        # Localized mousewheel scrolling
...
class MaterialManager:
    ...
    def create_widgets(self):
        ...
        # Tab 7: Sales Status (Dashboard)
        self.tab_sales_status = ttk.Frame(self.notebook)
        self.notebook.add(self.tab_sales_status, text='매출현황')
        self.setup_sales_status_tab()
        
        # Initial view update (Move here to ensure all tabs are ready before refresh)
        self.refresh_inquiry_filters()
        self.update_daily_usage_view()
        self.update_sales_status_view()
    ...
    def setup_inout_tab(self):
        ...
        self.ent_init.bind('<Return>', lambda e: btn_reg.focus_set())
        ...
        try:
            ...
            if hasattr(self, 'cb_trans_filter_vehicle'):
                ...
                if selected_vehicle and selected_vehicle != "전체":
                    df_to_show = df_to_show[df_to_show['차량번호'].astype(str).str.contains(selected_vehicle, na=False, case=False)]
    ...
    def setup_report_tab(self):
        ...
        self.cb_report_filter_year = ttk.Combobox(filter_frame, values=year_values, width=10)
        self.cb_report_filter_year.pack(side='left', padx=5)
        self.cb_report_filter_year.set(str(current_year))
        ...
        self.cb_report_filter_month.set(str(current_month))
        ...
        columns = ('연도', '월', '현장', '작업자', '작업시간', 'OT1', 'OT2', 'OT3', 'OT4', 'OT5', 'OT6', 'OT7', 'OT8', 'OT9', 'OT10', 
                   ...
                   '취급부주의', '고객불만', '기타', 'RTK총계', '형광자분', '흑색자분', '백색페인트', '침투제', '세척제', '현상제', '형광침투제', '비고')
    ...
    def setup_daily_usage_tab(self):
        ...
        self.ent_daily_date = DateEntry(form_content, width=12, date_pattern='yyyy-mm-dd', locale='ko_KR')
        self.ent_daily_date.grid(row=0, column=3, padx=2, pady=1, sticky='w')
        
        # [NEW] Focus Transition for Date
        def on_date_select(e):
            self.root.after(10, self.cb_daily_equip.focus_set)
            return "break"
        self.ent_daily_date.bind('<<DateEntrySelected>>', on_date_select)
        self.ent_daily_date.bind('<Return>', on_date_select)
    ...
    def setup_sales_status_tab(self):
        """Setup the sales status / dashboard tab"""
        # Main container
        main_frame = ttk.Frame(self.tab_sales_status, padding=10)
        main_frame.pack(fill='both', expand=True)

        # 1. Filter Section
        filter_frame = ttk.LabelFrame(main_frame, text="매출 조회 필터", padding=5)
        filter_frame.pack(fill='x', pady=(0, 10))

        ttk.Label(filter_frame, text="시작일:").pack(side='left', padx=5)
        self.sales_start_date = DateEntry(filter_frame, width=12, date_pattern='yyyy-mm-dd', locale='ko_KR')
        self.sales_start_date.pack(side='left', padx=5)
        # Default to 1st of current month
        today = datetime.datetime.now()
        start = today.replace(day=1)
        self.sales_start_date.set_date(start)

        ttk.Label(filter_frame, text="종료일:").pack(side='left', padx=5)
        self.sales_end_date = DateEntry(filter_frame, width=12, date_pattern='yyyy-mm-dd', locale='ko_KR')
        self.sales_end_date.pack(side='left', padx=5)
        self.sales_end_date.set_date(today)

        ttk.Label(filter_frame, text="현장명:").pack(side='left', padx=(15, 5))
        self.cb_sales_filter_site = ttk.Combobox(filter_frame, width=15)
        self.cb_sales_filter_site.pack(side='left', padx=5)
        self.cb_sales_filter_site.set('전체')
        
        # [FIX] Bind selection events for immediate search and autocomplete
        self.cb_sales_filter_site.bind("<<ComboboxSelected>>", lambda e: self.update_sales_status_view())
        self.cb_sales_filter_site.bind("<Return>", lambda e: self.update_sales_status_view())
        
        btn_query = ttk.Button(filter_frame, text="매출조회", command=self.update_sales_status_view)
        btn_query.pack(side='left', padx=10)
        
        btn_goto_daily = ttk.Button(filter_frame, text="상세 내역 조회", command=self.switch_to_daily_usage_details)
        btn_goto_daily.pack(side='left', padx=5)
        
        btn_export = ttk.Button(filter_frame, text="조회결과 엑셀 내보내기", command=self.export_sales_status)
        btn_export.pack(side='right', padx=5)

        # 2. KPI Summary Section
        kpi_row = ttk.Frame(main_frame)
        kpi_row.pack(fill='x', pady=(0, 10))
        
        def create_kpi_card(parent, title, attr_name, color="#2e7d32"):
            card = ttk.LabelFrame(parent, text=title, padding=10)
            card.pack(side='left', expand=True, fill='both', padx=5)
            val_label = ttk.Label(card, text="0", font=('Malgun Gothic', 16, 'bold'), foreground=color)
            val_label.pack()
            setattr(self, attr_name, val_label)
            return card

        create_kpi_card(kpi_row, "총 매출액 (합계)", "lbl_sales_total", "#d32f2f") # Red for total
        create_kpi_card(kpi_row, "순수 검사 수입", "lbl_sales_net")
        create_kpi_card(kpi_row, "현장 부대 비용 (출장/식대)", "lbl_sales_expense", "#1976d2") # Blue for expense
        create_kpi_card(kpi_row, "작업자 OT 비용", "lbl_sales_ot", "#f57c00") # Orange for OT

        # 3. Detail List Section
        detail_frame = ttk.LabelFrame(main_frame, text="매출 상세 내역", padding=5)
        ...
        # Columns matching the implementation plan
        columns = ('날짜', '현장명', '검사방법', '수량', '단가', '검사비', '출장비', '일식', 'OT금액', '합계', '비고')
        self.sales_tree = ttk.Treeview(tree_container, columns=columns, show='headings',
                                       yscrollcommand=vsb.set, xscrollcommand=hsb.set)
        ...
        col_widths = {
            '날짜': 100, '현장명': 130, '검사방법': 80, '수량': 70, '단가': 100, 
            '검사비': 100, '출장비': 90, '일식': 80, 'OT금액': 90, '합계': 110, '비고': 250
        }
        
        for col in columns:
            self.sales_tree.heading(col, text=col, command=lambda c=col: self.treeview_sort_column(self.sales_tree, c, False))
            width = col_widths.get(col, 100)
            self.sales_tree.column(col, width=width, minwidth=50, stretch=True, anchor='center')
    ...
    def update_sales_status_view(self):
        """Aggregate sales data and update KPIs and Treeview"""
        ...
        if hasattr(self, 'sales_tree'):
            self.sales_tree.focus_set()

        if self.daily_usage_df.empty:
            return

        # 1. Filter Data
        start_date = self.sales_start_date.get_date()
        end_date = self.sales_end_date.get_date()
        filter_site = self.cb_sales_filter_site.get().strip()

        # Convert daily_usage_df Date to datetime for comparison
        temp_df = self.daily_usage_df.copy()
        temp_df['Date'] = pd.to_datetime(temp_df['Date']).dt.date
        
        mask = (temp_df['Date'] >= start_date) & (temp_df['Date'] <= end_date)
        if filter_site != "전체":
            mask &= (temp_df['Site'] == filter_site)
        ...
        for _, row in filtered_df.iterrows():
            # [FIX] Collect and sum OT columns (OT, OT2, ..., OT10)
            ot_sum = 0.0
            for i in range(1, 11):
                key = 'OT' if i == 1 else f'OT{i}'
                ot_sum += to_f(row.get(key, 0))
            
            net = to_f(row.get('검사비', 0))
            travel = to_f(row.get('출장비', 0))
            meal = to_f(row.get('일식', 0))
            ot = ot_sum
            qty = to_f(row.get('검사량', 0)) # Fixed key: 수량 -> 검사량
            price = to_f(row.get('단가', 0))
            
            row_total = net + travel + meal + ot
            ...
            self.sales_tree.insert('', tk.END, values=tree_row)

        # 4. Update KPI Labels
        self.lbl_sales_total.config(text=f"{total_sales:,.0f} 원")
        self.lbl_sales_net.config(text=f"{total_net:,.0f} 원")
        self.lbl_sales_expense.config(text=f"{total_expense:,.0f} 원")
        self.lbl_sales_ot.config(text=f"{total_ot:,.0f} 원")

    def export_sales_status(self):
        """Export the currently filtered sales status to Excel"""
        ...
        summary_row['날짜'] = '합계'
        summary_row['검사비'] = self.lbl_sales_net.cget('text').replace(' 원', '').strip()
        summary_row['OT금액'] = self.lbl_sales_ot.cget('text').replace(' 원', '').strip()
        summary_row['합계'] = self.lbl_sales_total.cget('text').replace(' 원', '').strip()
    ...
    def add_daily_usage_entry(self):
        ...
        try:
            ...
            if self.save_data():
                ...
                self.update_report_view() # Also refresh report view
    ...
    def refresh_inquiry_filters(self):
        ...
        unique_sites = sorted([s for s in raw_sites if s and str(s).lower() != 'nan'])
    ...
    def update_daily_usage_view(self):
        ...
        if filter_vehicle != '전체' and '차량번호' in filtered_df.columns:
            ...
            filtered_df = filtered_df[filtered_df['차량번호'].astype(str).str.contains(filter_vehicle, na=False, case=False)]
        ...
        try:
            ...
            if (hasattr(self, 'tab_daily_usage') and str(current_tab) == str(self.tab_daily_usage)) or \
               tab_text == '현장별 일일 사용량 기입':
                ...
                print("Report tab selected - refreshing view")
                self.update_report_view()
        except Exception as e:
