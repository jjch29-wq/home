            ...
            if '주' in qty_str:
                ...
                try:
                    ...
                    if pos_pct <= 25:
                        ...
                        msg += f"  • 📅 2주 단기 전망: 과매도 구간으로 1~2주 내에 저가 매수세가 유입되며 단기 반등이 나타날 확률이 높습니다. 매수 타이밍을 잴 시간적 여유가 있습니다.\n"
                        msg += f"  • 액션 플랜 (STRONG HOLD / BUY): 절대 매도 금지! 통계적 반등 구간입니다. {fmt(support)} 부근에서 추가 매수를 고려해보세요.\n"
                    elif pos_pct <= 40:
                        msg += f"  • 현재 상태: [바닥 다지기 및 반등 시작 (하위 {int(pos_pct)}%)]\n"
                        msg += f"  • 📅 2주 단기 전망: 향후 2주간 서서히 저점을 높여가며 안정적인 우상향 궤도에 진입할 것으로 예상됩니다.\n"
                        msg += f"  • 액션 플랜 (HOLD): 상승 추세 전환 성공! 보유를 유지하시고, 목표가 {fmt(resistance)}를 기다리세요.\n"
                    elif pos_pct <= 70:
                        msg += f"  • 현재 상태: [중간 상승 구간 (상위 {100-int(pos_pct)}%)]\n"
                        msg += f"  • 📅 2주 단기 전망: 1~2주 내 전고점 돌파를 시도할 가능성이 높으나, 단기 박스권 흐름이 연출될 수 있습니다.\n"
                        msg += f"  • 액션 플랜 (WATCH): 순조롭게 수익 중. 향후 1~2주 내로 저항선에 가까워지면 분할 매도를 준비할 시기입니다.\n"
                    else:
                        msg += f"  • 현재 상태: [고점 도달 (상위 {100-int(pos_pct)}%)]\n"
                        msg += f"  • 📅 2주 단기 전망: 단기 과열 구간으로, 2주 이내에 차익 실현 매물이 쏟아지며 하락 조정(Pullback) 위험이 큽니다.\n"
                        msg += f"  • 액션 플랜 (TAKE PROFIT / SELL): 목표가({fmt(resistance)}) 근접! 조정이 임박했으니 이번 주 내로 빠르게 수익 실현을 권장합니다.\n"
