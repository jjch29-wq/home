...
class KogasDailyWorkLogExporter:
    def __init__(self):
        # Define basic styles
    ...
    def generate_excel(self, data, output_path):
        ...
        # Left Info
        merge_and_set('A4:F4', "용 역 명 : 가산~가평 천연가스 공급시설 건설공사 비파괴검사기술용역", font=self.font_normal, align=self.align_left, border=None)
        ...
        equip_rows = ['RT장비(선원)', 'UT장비', 'MT/PT장비']
        ...
        merge_and_set('R26:R27', 'UT(m)', font=self.font_small, fill=self.fill_header)
        ...
        for i in range(total_rows): # minimum 10 empty rows for good look
            ...
            set_cell(f'K{row_idx}', sec_info[1] if method not in ['UT', 'PT', 'MT'] and sec_info_raw != "관련지시 없음" else '')
            set_cell(f'L{row_idx}', sec_info[2] if method not in ['UT', 'PT', 'MT'] and sec_info_raw != "관련지시 없음" else '')
            set_cell(f'M{row_idx}', sec_info[3] if method not in ['UT', 'PT', 'MT'] and sec_info_raw != "관련지시 없음" else '')
            
            if method in ['UT', 'PT', 'MT'] or sec_info_raw == "관련지시 없음":
