...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "kogas_config.json")
...
DEFAULT_CONFIG = {
    ...
    "CONTRACT_QTY": {
        "주배관": {
            "일반": {"RT_B": 14205, "RT_A": 0, "RT_A2": 0, "UT": 237.66, "PT": 237.66},
            "야간": {"RT_B": 3345, "RT_A": 0, "RT_A2": 0, "UT": 54.24, "PT": 54.23},
            "휴일": {"RT_B": 1575, "RT_A": 0, "RT_A2": 0, "UT": 27.12, "PT": 27.12}
        },
        "플랜트(관리소)": {
            "일반": {"RT_B": 986, "RT_A": 1969, "RT_A2": 1359, "UT": 0, "PT": 15.4},
            "야간": {"RT_B": 131, "RT_A": 256, "RT_A2": 168, "UT": 0, "PT": 2.11},
            "휴일": {"RT_B": 126, "RT_A": 239, "RT_A2": 177, "UT": 0, "PT": 2.11}
        }
    }
}

def _normalize_config(config):
    """설정 파일의 키 이름이 잘못된 경우(예: 열배관) 주배관으로 자동 수정"""
    def fix_keys(d, old_key, new_key):
        if old_key in d:
    ...
    if "LABOR_COST" in config:
        config["LABOR_COST"] = fix_keys(config["LABOR_COST"], "열배관", "주배관")
    # CONTRACT_QTY 키 정규화
    if "CONTRACT_QTY" in config:
        config["CONTRACT_QTY"] = fix_keys(config["CONTRACT_QTY"], "열배관", "주배관")
    
    # DEFAULT_CONFIG 기준 누락된 키 보충
    for loc in DEFAULT_CONFIG["LABOR_COST"]:
        if loc not in config.get("LABOR_COST", {}):
            config.setdefault("LABOR_COST", {})[loc] = DEFAULT_CONFIG["LABOR_COST"][loc]
    for loc in DEFAULT_CONFIG["CONTRACT_QTY"]:
        if loc not in config.get("CONTRACT_QTY", {}):
            config.setdefault("CONTRACT_QTY", {})[loc] = DEFAULT_CONFIG["CONTRACT_QTY"][loc]
    return config
...
class NDTCalculatorTab(ttk.Frame):
    def __init__(self, parent, main_app=None):
        ...
        self.auto_load_contract_qty(show_popup=False)
    ...
    def create_widgets(self):
        ...
        self.tech_fee_rate_var = tk.DoubleVar(value=5.8606)
        ...
        content_frame.columnconfigure(0, weight=1)   # Left frame: 남은 공간 전체 확장
        content_frame.columnconfigure(1, weight=0, minsize=380)  # Right frame: 고정 380px
        ...
        contract_hscrollbar = ttk.Scrollbar(contract_frame, orient="horizontal", command=contract_canvas.xview)
        contract_canvas.configure(yscrollcommand=contract_scrollbar.set, xscrollcommand=contract_hscrollbar.set)
        
        contract_scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        contract_hscrollbar.pack(side=tk.BOTTOM, fill=tk.X)
        ...
        def _on_frame_configure(e):
            contract_canvas.configure(scrollregion=contract_canvas.bbox("all"))
        def _on_canvas_configure(e):
            # 내용이 캔버스보다 좁으면 캔버스 폭에 맞게, 넓으면 스크롤
            frame_req_w = contract_inner_frame.winfo_reqwidth()
            canvas_w = e.width
            contract_canvas.itemconfig(contract_window, width=max(canvas_w, frame_req_w))
        contract_inner_frame.bind("<Configure>", _on_frame_configure)
        contract_canvas.bind("<Configure>", _on_canvas_configure)
        
        self.contract_vars = {}
        
        headers = ["구간", "시간", "항목", "단위", "계약수량", "전회수량", "금회수량", "잔여수량", "계약단가", "계약금액", "전회금액"]
        for i, h in enumerate(headers):
            ttk.Label(contract_inner_frame, text=h, font=("Arial", 9, "bold"), anchor="center").grid(row=0, column=i, padx=2, pady=2, sticky="ew")
        
        # 모든 열에 공간을 골고루 배분하여 어느 한 곳만 비어보이지 않게 설정
        for col_idx in range(11):
            contract_inner_frame.columnconfigure(col_idx, weight=1)
        # 내용이 긴 항목과 금액 열만 조금 더 넓게
        contract_inner_frame.columnconfigure(2, weight=2)   # 항목
        contract_inner_frame.columnconfigure(9, weight=2)   # 계약금액
        contract_inner_frame.columnconfigure(10, weight=2)  # 전회금액

        locations = ["주배관", "플랜트(관리소)"]
        times = ["일반", "야간", "휴일"]
        materials = [
            ("RT_B", 'RT (B필름: 3⅓"x17")'),
            ("RT_A", 'RT (A필름: 3⅓"x12")'),
        ...
        for loc in locations:
            for t_time in times:
                for m_key, m_name in materials:
                    ...
                    # 구간: 해당 위치 첫 번째 행에만 표시
                    loc_text = loc if (t_time == "일반" and m_key == "RT_B") else ""
                    # 시간: 해당 시간대 첫 번째 항목에만 표시
                    time_text = t_time if m_key == "RT_B" else ""
                    
                    ttk.Label(contract_inner_frame, text=loc_text, font=("Arial", 9, "bold"), anchor="center").grid(row=row_idx, column=0, sticky="ew", padx=2)
                    ttk.Label(contract_inner_frame, text=time_text, font=("Arial", 9), anchor="center").grid(row=row_idx, column=1, sticky="ew", padx=2)
                    ttk.Label(contract_inner_frame, text=m_name, font=("Arial", 9), anchor="center").grid(row=row_idx, column=2, sticky="ew", padx=2)
                    ttk.Label(contract_inner_frame, text=unit, font=("Arial", 9), anchor="center").grid(row=row_idx, column=3, padx=2)
                    ...
                    unit_cost = 0
                    try:
                        lab_type = m_key.split('_')[0] if "_" in m_key else m_key
                        lab_unit = LABOR_COST[loc][t_time].get(lab_type, 0)
                        mat_unit = MATERIAL_COST.get(m_name, 0)
                        ...
                        c_price_var.set(f"{int(unit_cost):,}")
                    except Exception as e:
                        print(e)
                        pass
                    ...
                    ttk.Entry(contract_inner_frame, textvariable=c_qty, width=7).grid(row=row_idx, column=4, padx=1, pady=1)
                    ttk.Entry(contract_inner_frame, textvariable=p_qty, width=7).grid(row=row_idx, column=5, padx=1, pady=1)
                    ...
                    lbl_rem = ttk.Label(contract_inner_frame, textvariable=rem_qty, width=7, anchor="e", font=("Arial", 9, "bold"))
                    lbl_rem.grid(row=row_idx, column=7, padx=1, pady=1)
                    
                    ttk.Label(contract_inner_frame, textvariable=c_price_var, anchor="e", width=9).grid(row=row_idx, column=8, padx=1, pady=1)
                    ttk.Entry(contract_inner_frame, textvariable=c_var, width=13).grid(row=row_idx, column=9, padx=1, pady=1, sticky="ew")
                    ttk.Entry(contract_inner_frame, textvariable=p_var, width=11).grid(row=row_idx, column=10, padx=1, pady=1, sticky="ew")
        ...
        items = [
            ...
            ("travel", "주재비 및 출장여비", 2226000),
            ("print", "도서인쇄비", 481000),
            ("liability", "엔지니어링 손해배상 공제료", 14729000)
        ]
        ...
        # 기타 경비 합계 행 추가
        tf = ttk.Frame(exp_frame)
        tf.pack(fill=tk.X, pady=(10, 2))
        
        ttk.Label(tf, text="[경비 소계]", width=18, font=("Arial", 9, "bold")).grid(row=0, column=0, padx=2)
        
        self.exp_subtotal_budget = tk.StringVar(value="0")
        self.exp_subtotal_prev = tk.StringVar(value="0")
        self.exp_subtotal_curr = tk.StringVar(value="0")
        self.exp_subtotal_rem = tk.StringVar(value="0")
        
        ttk.Label(tf, textvariable=self.exp_subtotal_budget, width=15, anchor="e", font=("Arial", 9, "bold")).grid(row=0, column=1, padx=(2, 6))
        ttk.Label(tf, textvariable=self.exp_subtotal_prev, width=15, anchor="e", font=("Arial", 9, "bold")).grid(row=0, column=2, padx=(2, 6))
        ttk.Label(tf, textvariable=self.exp_subtotal_curr, width=15, anchor="e", font=("Arial", 9, "bold")).grid(row=0, column=3, padx=(2, 6))
        lbl_rem_sub = ttk.Label(tf, textvariable=self.exp_subtotal_rem, width=12, anchor="e", font=("Arial", 9, "bold"), foreground="blue")
        lbl_rem_sub.grid(row=0, column=4, padx=2)

        # 총합계 행 추가 (손해배상 포함)
        tf_total = ttk.Frame(exp_frame)
        tf_total.pack(fill=tk.X, pady=(2, 10))
        
        ttk.Label(tf_total, text="[총 실비정산 합계]", width=18, font=("Arial", 9, "bold"), foreground="red").grid(row=0, column=0, padx=2)
        
        self.exp_total_budget = tk.StringVar(value="0")
        self.exp_total_prev = tk.StringVar(value="0")
        self.exp_total_curr = tk.StringVar(value="0")
        self.exp_total_rem = tk.StringVar(value="0")
        
        ttk.Label(tf_total, textvariable=self.exp_total_budget, width=15, anchor="e", font=("Arial", 9, "bold"), foreground="red").grid(row=0, column=1, padx=(2, 6))
        ttk.Label(tf_total, textvariable=self.exp_total_prev, width=15, anchor="e", font=("Arial", 9, "bold"), foreground="red").grid(row=0, column=2, padx=(2, 6))
        ttk.Label(tf_total, textvariable=self.exp_total_curr, width=15, anchor="e", font=("Arial", 9, "bold"), foreground="red").grid(row=0, column=3, padx=(2, 6))
        lbl_rem_tot = ttk.Label(tf_total, textvariable=self.exp_total_rem, width=12, anchor="e", font=("Arial", 9, "bold"), foreground="red")
        lbl_rem_tot.grid(row=0, column=4, padx=2)
        
        def update_exp_totals(*args):
            t_b, t_p, t_c, t_r = 0, 0, 0, 0
            sub_b, sub_p, sub_c, sub_r = 0, 0, 0, 0
            for k, v in self.exp_vars.items():
                b = self.get_int(v["budget"])
                p = self.get_int(v["prev"])
                try: c = float(v["curr"].get())
                except: c = 0
                r = self.get_float(v["rem"])
                
                t_b += b; t_p += p; t_c += c; t_r += r
                
                if k != "liability":
                    sub_b += b; sub_p += p; sub_c += c; sub_r += r
                    
            self.exp_subtotal_budget.set(f"{int(sub_b):,}")
            self.exp_subtotal_prev.set(f"{int(sub_p):,}")
            self.exp_subtotal_curr.set(f"{int(sub_c):,}" if float(sub_c).is_integer() else f"{sub_c:,.2f}")
            self.exp_subtotal_rem.set(f"{int(sub_r):,}" if float(sub_r).is_integer() else f"{sub_r:,.2f}")
            if sub_r < 0: lbl_rem_sub.config(foreground="red")
            else: lbl_rem_sub.config(foreground="blue")

            self.exp_total_budget.set(f"{int(t_b):,}")
            self.exp_total_prev.set(f"{int(t_p):,}")
            self.exp_total_curr.set(f"{int(t_c):,}" if float(t_c).is_integer() else f"{t_c:,.2f}")
            self.exp_total_rem.set(f"{int(t_r):,}" if float(t_r).is_integer() else f"{t_r:,.2f}")
            
        for k, v in self.exp_vars.items():
            v["budget"].trace_add("write", update_exp_totals)
            v["prev"].trace_add("write", update_exp_totals)
            v["curr"].trace_add("write", update_exp_totals)
            v["rem"].trace_add("write", update_exp_totals)
    ...
    def update_dynamic_ui(self, *args):
        ...
        if ndt_type == "RT":
            ...
            if self.material_var.get() not in ['RT (B필름: 3⅓"x17")', 'RT (A필름: 3⅓"x12")', 'RT (A/2필름: 3⅓"x6")']:
                self.material_var.set('RT (B필름: 3⅓"x17")')
        elif ndt_type == "UT":
            self.material_combo.config(values=["UT"], state="disabled")
            self.material_var.set("UT")
        elif ndt_type == "PT":
    ...
    def _do_calculate(self):
        ...
        loc_type_val = loc_type.get() if loc_type else "주배관"
    ...
    def update_qty_summary(self):
        ...
        for rec in self.records:
            loc = "플랜트(관리소)" if "관리소" in rec["loc"] or "플랜트" in rec.get("loc_type", rec["loc"]) else "주배관"
    ...
    def carry_over_round(self):
        ...
        for cat, v in self.contract_vars.items():
            ...
            for r in target_records:
                loc = "플랜트(관리소)" if "관리소" in r["loc"] or "플랜트" in r.get("loc_type", r["loc"]) else "주배관"
    ...
    def auto_load_contract_qty(self, show_popup=True):
        ...
        for loc in contract_qtys:
            for t_time in contract_qtys[loc]:
                if isinstance(contract_qtys[loc][t_time], dict):
                    ...
                    # Flat structure (e.g. loc -> mat) mapped to '일반'
                    mat = t_time
    ...
    def load_project(self):
        try:
            ...
            self.exp_vars["print"]["budget"].set(f"{ex.get('print_budget', 2226000):,}")
    ...
    def export_to_excel(self):
        ...
        try:
            ...
            ws.Range("C4").Value = "가스공사 가산~가평 비파괴검사용역 단가계약"
            ...
            for g_type in categories_det:
                ...
                for key, data in aggregated.items():
                    ...
                    loc_val = "플랜트(관리소)" if "관리소" in key[0] or "플랜트" in key[0] else "주배관"
                    t_time = key[3]
                    mat_raw = key[2]
                    ndt_type = key[1]
