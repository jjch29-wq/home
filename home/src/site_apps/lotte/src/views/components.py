...
import traceback
import json
from site_apps.lotte.src.utils.helpers import normalize_id, enable_column_resize
...
class LaborCostDetailWidget(ttk.Frame):
    ...
    def apply_base_salaries(self):
        ...
        for rank, salary in self.base_salaries.items():
            if rank in self.entries:
                daily_rate = salary / 240 if self.exact_rates else round(salary / 240)
    ...
    def _on_input_change(self, key):
        ...
        price = self._calculation_price(key)
    ...
    def calculate_all(self):
        ...
        for rank in self.ranks:
            ...
            c = p * d * self._calculation_price(rank)
            t1_personnel += p
            t1_days += d
            t1_cost += c
        ...
        for stype in self.special_types:
            ...
            c = p * h * self._calculation_price(stype)
    ...
    def _calculation_price(self, key):
        """Use the workbook's unrounded rate while its displayed default is unchanged."""
        entered = self._to_f(self.entries[key]['unit_price'].get())
        if self.exact_rates and self.default_labor and key in self.default_labor:
            raw = self._to_f(self.default_labor[key].get('unit_price', 0))
            if entered == round(raw):
                return raw
        return entered
...
class ExpenseProfitDetailWidget(ttk.Frame):
    ...
    def _create_widgets(self):
        ...
        ttk.Label(insurance_table, text="10.66608%" if self.budget_mode == 'planned' else "10.6661%", relief='solid', padding=5, anchor='center').grid(row=1, column=3, sticky='nsew')
    ...
    def calculate_all(self, event=None):
        ...
        t4 = labor_total * (0.1066608 if self.budget_mode == 'planned' else 0.106661)
        ...
        self.raw_expense_total = exp_total
        displayed_expense = exp_total + t3 if self.budget_mode == 'planned' else exp_total
        self.lbl_exp_total.config(text=f"₩ {displayed_expense:,.0f}")
    ...
    def get_total_cost(self):
        ...
        if self.budget_mode == 'planned':
            return getattr(self, 'raw_expense_total', 0.0)
        try:
