...
from site_apps.lotte.src.utils.helpers import install_and_import, normalize_id
...
from site_apps.lotte.src.daily_work_report_manager import DailyWorkReportManager
from site_apps.lotte.src.ndt_billing_tab import NDTCalculatorTab
from site_apps.lotte.src.daily_work_log_tab import DailyWorkLogTab
import site_apps.lotte.src.daily_work_report_manager as daily_work_report_manager
...
class MaterialManager:
    # ExpenseProfitDetailWidget에서 롯데 실행예산서 전용 계산식을 선택한다.
    lotte_budget_rules = True
    ...
    def __init__(self, root):
        ...
        self.data_dir = os.path.join(appdata_dir, 'PMI_Apps', 'Lotte')
        os.makedirs(self.data_dir, exist_ok=True)
        
        self.config_path = os.path.join(self.data_dir, 'Lotte_Material_Manager_Config.json')

        # 스크립트 실행 모드: src/ 우선, 없으면 ../data/ 탐색 (For DB)
        _db_name = 'Lotte_Material_Inventory.xlsx'
        _data_candidates = [
            os.path.join(self.data_dir, _db_name),           # 1. %APPDATA%
            os.path.join(self.app_dir, _db_name),            # 2. src/
            os.path.join(os.path.dirname(self.app_dir), 'data', _db_name), # 3. data/
        ]
        ...
        if getattr(sys, 'frozen', False):
            ...
            self.db_path = os.path.join(self.app_dir, 'Lotte_Material_Inventory.xlsx')
    ...
    def _on_daily_usage_sash_changed(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_daily_usage_sash_changed_impl
    ...
    def _sync_dataframe_schema(self, *args, **kwargs):
        from site_apps.lotte.src.models.material_model import _sync_dataframe_schema_impl
        return _sync_dataframe_schema_impl(self, *args, **kwargs)

    def load_data(self, *args, **kwargs):
        if "export" in "load_data" or "excel" in "load_data":
            from site_apps.lotte.src.services.excel_exporter import load_data_impl
        else:
            from site_apps.lotte.src.services.data_loader import load_data_impl
    ...
    def clean_nan(self, *args, **kwargs):
        from site_apps.lotte.src.utils.helpers import clean_nan_impl
    ...
    def save_data(self, *args, **kwargs):
        ...
        if "export" in "save_data" or "excel" in "save_data":
            from site_apps.lotte.src.services.excel_exporter import save_data_impl
        else:
            from site_apps.lotte.src.services.data_loader import save_data_impl
    ...
    def get_base_salaries(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import get_base_salaries_impl
        return get_base_salaries_impl(self, *args, **kwargs)

    def get_material_defaults(self, *args, **kwargs):
        from site_apps.lotte.src.models.material_model import get_material_defaults_impl
    ...
    def setup_stock_tab(self, *args, **kwargs):
        from site_apps.lotte.src.views.stock_management_view import setup_stock_tab_impl
    ...
    def calculate_current_stock(self, *args, **kwargs):
        from site_apps.lotte.src.models.material_model import calculate_current_stock_impl
        return calculate_current_stock_impl(self, *args, **kwargs)

    def update_stock_view(self, *args, **kwargs):
        from site_apps.lotte.src.views.stock_management_view import update_stock_view_impl
    ...
    def _apply_combobox_word_suggest(self, combobox, source_values, open_dropdown=False):
        ...
        text = text_raw.strip()
        combobox._last_typed_text = text_raw
        had_focus = False
        try:
        ...
        if getattr(combobox, '_allow_multi', False) and ',' in text:
            q = text.rsplit(',', 1)[-1].strip().lower()
        else:
            q = text.lower()
    ...
    def _bind_combobox_word_suggest(self, combobox, source_getter):
        ...
        def _on_selected(e=None):
            ...
            if getattr(combobox, '_allow_multi', False) and hasattr(combobox, '_last_typed_text'):
                last_text = getattr(combobox, '_last_typed_text', '').strip()
                selected_item = combobox.get().strip()
                
                prefix = last_text.rsplit(',', 1)[0].strip() + ', ' if ',' in last_text else ''
                last_segment = last_text.rsplit(',', 1)[-1].strip() if ',' in last_text else last_text
                
                if last_segment and last_segment.lower() not in selected_item.lower():
                    # User is selecting a new item without explicitly typing a comma
                    combobox.set(last_text + ', ' + selected_item if last_text else selected_item)
                else:
                    # User is autocompleting the last segment
                    combobox.set(prefix + selected_item)
        ...
        def _on_click(e=None):
            combobox._last_typed_text = combobox.get()
    ...
    def on_material_selected(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_material_selected_impl
        return on_material_selected_impl(self, *args, **kwargs)

    def _on_trans_site_return(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_trans_site_return_impl
        return _on_trans_site_return_impl(self, *args, **kwargs)

    def _on_warehouse_return(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_warehouse_return_impl
        return _on_warehouse_return_impl(self, *args, **kwargs)

    def _on_user_return(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_user_return_impl
        return _on_user_return_impl(self, *args, **kwargs)


    def format_worker_summary(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import format_worker_summary_impl
    ...
    def save_df_to_excel_autofit(self, *args, **kwargs):
        if "export" in "save_df_to_excel_autofit" or "excel" in "save_df_to_excel_autofit":
            from site_apps.lotte.src.services.excel_exporter import save_df_to_excel_autofit_impl
        else:
            from site_apps.lotte.src.services.data_loader import save_df_to_excel_autofit_impl
    ...
    def export_materials(self, *args, **kwargs):
        if "export" in "export_materials" or "excel" in "export_materials":
            from site_apps.lotte.src.services.excel_exporter import export_materials_impl
        else:
            from site_apps.lotte.src.services.data_loader import export_materials_impl
    ...
    def setup_monthly_usage_tab(self, *args, **kwargs):
        from site_apps.lotte.src.views.monthly_usage_view import setup_monthly_usage_tab_impl
        return setup_monthly_usage_tab_impl(self, *args, **kwargs)
    
    def update_monthly_usage_view(self, *args, **kwargs):
        from site_apps.lotte.src.views.monthly_usage_view import update_monthly_usage_view_impl
        return update_monthly_usage_view_impl(self, *args, **kwargs)

    def on_monthly_usage_select(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_monthly_usage_select_impl
    ...
    def open_list_management_dialog(self, title_or_key, data_list=None, config_key=None, target_cb=None):
        ...
        def select_and_close():
            ...
            if target_cb:
                # [NEW] Multi-select append logic
                if getattr(target_cb, '_allow_multi', False) and hasattr(target_cb, 'get'):
                    last_text = target_cb.get().strip()
                    if last_text:
                        last_segment = last_text.rsplit(',', 1)[-1].strip() if ',' in last_text else last_text
                        if last_segment.lower() not in val.lower():
                            val = last_text + ', ' + val
                        else:
                            prefix = last_text.rsplit(',', 1)[0].strip() + ', ' if ',' in last_text else ''
                            val = prefix + val
    ...
    def refresh_ui_for_list_change(self, config_key):
        ...
        if config_key == 'sites':
            ...
            for i in range(1, 11):
                ...
                # Also update via worker_group if available
    ...
    def on_drag_stop(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_drag_stop_impl
    ...
    def on_drag_start(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_drag_start_impl
        return on_drag_start_impl(self, *args, **kwargs)
        
    def on_resize_start(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_resize_start_impl
        return on_resize_start_impl(self, *args, **kwargs)
        
    def on_mouse_motion(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_mouse_motion_impl
    ...
    def setup_daily_usage_tab(self, *args, **kwargs):
        from site_apps.lotte.src.views.daily_usage_view_lotte import setup_daily_usage_tab_impl
        return setup_daily_usage_tab_impl(self, *args, **kwargs)

    def on_recent_record_click(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_recent_record_click_impl
    ...
    def _on_daily_usage_select(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_daily_usage_select_impl
    ...
    def export_budget_estimation(self):
        ...
        try:
            from site_apps.lotte.src.utils.export_helper import export_budget_estimation_to_excel
    ...
    def _load_budget_to_form(self, site, silent=False):
        ...
        def _normalize_lotte_outsource(data):
            """사전/사후원가의 예전 외주 행을 롯데 현장 표준 업체로 교체한다."""
            if not isinstance(data, dict):
                data = {}
            old_rows = data.get('outsource', [])
            if not isinstance(old_rows, list):
                old_rows = []
            old_by_name = {
                str(item.get('cat', '')).strip(): item
                for item in old_rows if isinstance(item, dict)
            }
            standard_rows = []
            for name, price in (
                ('케이엔디이', 15000),
                ...
                standard_rows.append({
                    ...
                    'count': old.get('count', ''),
                    'price': f'{price:,}',
                    'amount': old.get('amount', '0'),
                })
            data['outsource'] = standard_rows
            return data
            ...
            if widget:
                    ...
                    if p_jdata and str(p_jdata).strip() not in ['', '{}', '[]']:
                        try:
                            ...
                            if 'Labor' in json_col and isinstance(p_data, dict):
                                ...
                                if json_col == 'Actual_ExpenseDetail':
                                    p_data = _normalize_lotte_outsource(p_data)
                                    fixed_actual_counts = {
                                        '케이엔디이': '3918',
                                        '고려검사': '651',
                                        '한국기계검사소': '453',
                                    }
                                    for out_row in p_data['outsource']:
                                        name = str(out_row.get('cat', '')).strip()
                                        out_row['count'] = fixed_actual_counts.get(name, '')
                ...
                if jdata and str(jdata).strip() not in ['', '{}', '[]']:
                    try:
                        ...
                        if json_col in ('ExpenseDetail', 'Actual_ExpenseDetail'):
                            loaded_data = _normalize_lotte_outsource(loaded_data)
                            if json_col == 'ExpenseDetail':
                                # 원본 사전원가 G69: 2,084매
                                loaded_data['outsource'][0]['count'] = '2084'
                            else:
                                # 업체별 기성서 검토 결과에 따른 사후원가 고정 수량
                                fixed_actual_counts = {
                                    '케이엔디이': '3918',
                                    '고려검사': '651',
                                    '한국기계검사소': '453',
                                }
                                for out_row in loaded_data['outsource']:
                                    name = str(out_row.get('cat', '')).strip()
                                    out_row['count'] = fixed_actual_counts.get(name, '')
    ...
    def fill_budget_from_actuals(self):
        ...
        def _row_has_worker(r):
            """User~User10 중 실제 작업자명이 하나라도 있는 내부 작업 행인지 판정."""
            for i in range(1, 11):
                col = 'User' if i == 1 else f'User{i}'
                worker = str(r.get(col, '')).strip().lower()
                if worker not in ('', 'nan', 'none', 'null'):
                    return True
            return False

        # 작업자 없는 행은 외주 작업이므로 내부 재료/경비/장비 집계에서 완전히 제외한다.
        internal_df = df[df.apply(_row_has_worker, axis=1)].copy() if not df.empty else df.copy()
        total_days_count = internal_df['_date_ts'].dt.date.nunique() if not internal_df.empty else 0
        ...
        for _, row in df.iterrows():
            ...
            nd_overhead = _f(row.get('제경비', 0))
            nd_tech = _f(row.get('기술료', 0))
            # 작업자 유무 확인: User, User2 ~ User10 중 하나라도 값이 있으면 인건비, 없으면 외주비
            has_worker = _row_has_worker(row)
            if has_worker:
                lab_total += nd_labor
            row_revenue = nd_labor + nd_mat + nd_overhead + nd_tech
            if row_revenue <= 0:
            ...
            # 외주 행은 매출과 외주비에만 반영한다. 내부 자재·경비·장비에는 반영하지 않는다.
            if not has_worker:
                continue
        ...
        # 외주비는 사후원가 상세의 고정 입력값을 사용한다.
        # 현장별 탭의 작업자 없는 행으로 자동 갱신하지 않는다.
        ...
        if hasattr(self, 'actual_expense_detail_widget'):
            ...
            for i, row_data in enumerate(current_exp_data.get('site_expense', [])):
                ...
                if any(k in cat for k in ['출장비', '소모품비', '복리후생비', '복리', '후생', '식대']):
            ...
            # 먼저 set_data로 site_expense/rental/depreciation 전체 반영
            self.actual_expense_detail_widget.set_data(current_exp_data)

            # 외주비/잡급 3개 행은 사용자가 입력하고 저장한 값을 그대로 유지한다.
            # 현장별 탭의 작업자 없는 행은 여기로 추가하거나 금액을 덮어쓰지 않는다.
    ...
    def on_budget_tree_select(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_budget_tree_select_impl
    ...
    def _on_daily_usage_resize(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_daily_usage_resize_impl
    ...
    def _on_main_window_resize(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import _on_main_window_resize_impl
    ...
    def on_daily_usage_tree_select(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_daily_usage_tree_select_impl
    ...
    def format_entry_with_commas(self, *args, **kwargs):
        from site_apps.lotte.src.utils.helpers import format_entry_with_commas_impl
    ...
    def sync_worker_times(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import sync_worker_times_impl
    ...
    def export_daily_work_report(self, *args, **kwargs):
        if "export" in "export_daily_work_report" or "excel" in "export_daily_work_report":
            from site_apps.lotte.src.services.excel_exporter import export_daily_work_report_impl
        else:
            from site_apps.lotte.src.services.data_loader import export_daily_work_report_impl
        return export_daily_work_report_impl(self, *args, **kwargs)

    def export_central_daily_work_report(self, *args, **kwargs):
        from site_apps.lotte.src.services.excel_exporter import export_central_daily_work_report_impl
    ...
    def update_daily_usage_view(self, *args, **kwargs):
        from site_apps.lotte.src.views.daily_usage_view_lotte import update_daily_usage_view_impl
    ...
    def _calculate_ot_from_worktime(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import _calculate_ot_from_worktime_impl
    ...
    def _is_consumable_material(self, *args, **kwargs):
        from site_apps.lotte.src.models.material_model import _is_consumable_material_impl
        ...
        def do_export():
            try:
                ...
                try:
                    ...
                    from site_apps.lotte.src.tagged_ndt_writer import write_all_tagged_sections
    ...
    def on_tab_drag_start(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_tab_drag_start_impl
        return on_tab_drag_start_impl(self, *args, **kwargs)

    def on_tab_drag(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_tab_drag_impl
        return on_tab_drag_impl(self, *args, **kwargs)

    def normalize_site_name(self, *args, **kwargs):
        from site_apps.lotte.src.utils.helpers import normalize_site_name_impl
        return normalize_site_name_impl(self, *args, **kwargs)

    def on_tab_drag_end(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_tab_drag_end_impl
        return on_tab_drag_end_impl(self, *args, **kwargs)
    
    def on_tab_changed(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_tab_changed_impl
    ...
    def calculate_ot_amount(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import calculate_ot_amount_impl
        return calculate_ot_amount_impl(self, *args, **kwargs)

    def _parse_ot_hours(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import _parse_ot_hours_impl
        return _parse_ot_hours_impl(self, *args, **kwargs)

    def _calculate_split_ot_hours(self, *args, **kwargs):
        from site_apps.lotte.src.models.worker_model import _calculate_split_ot_hours_impl
    ...
    def load_tab_config(self):
        ...
        try:
            ...
            if os.path.exists(self.config_path):
                ...
                from site_apps.lotte.src.views.components import VehicleInspectionWidget
    ...
    def on_closing(self, *args, **kwargs):
        from site_apps.lotte.src.controllers.event_controller import on_closing_impl
