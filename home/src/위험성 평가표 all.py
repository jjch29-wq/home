...
center_nowrap_align = Alignment(horizontal="center", vertical="center", wrap_text=False, shrink_to_fit=True)
...
def get_risk_category(text):
    ...
    if any(w in text for w in ["방사능", "방사선", "피폭", "소음", "진동", "조도", "폭염", "한파", "고온", "저온"]): return "물리적 요인"
...
def create_excel(process_name, output_filename, data, params):
    ...
    widths = [14.0, 10.5, 51, 9, 5, 5, 24] # A:G (A 증가, B 감소)
    widths += [7]                     # H
    widths += [8.2] * 7               # I~O
    widths += [7]                     # P
    ...
    for item in data:
        ...
        ws.cell(row=current_row, column=1, value=item[0]).alignment = center_nowrap_align
        ...
        if count > 1:
            ...
            while current_start <= current_end:
                ...
                if current_start > a_r:
                    ws.cell(row=current_start, column=1, value=cat).alignment = center_nowrap_align
...
data_rt = [
    ...
    ["사전준비", "차폐체 설치/해체 작업 시 중량물 낙하에 의한 사고 위험", "납 차폐체 수작업 이동 시 반드시 2인 1조 협력 운반 및 작업화(안전화) 착용 상태 점검", 2, 3, "V", "1. 100kg 이상 인양 시 2줄 걸이 이상 결속\n2. 인양 중인 중량물 하부 통행 전면 금지"],
    ...
    ["작업환경", "폭염 시 야외 작업 중 열사병·열탈진 등 온열질환 발생 위험", "작업 전 기상청 폭염 특보 확인 및 이상 시 즉시 대피", 2, 2, "IV", "1. 폭염 특보(35°C 이상) 발효 시 작업 일정 조정 및 오후 2~5시 야외 작업 최소화\n2. 시원한 휴식공간 마련 및 물·이온음료 상시 비치\n3. 작업자 1인당 1시간 이상 휴식 부여 및 현장 책임자 주기 순찰"],
    ["작업환경", "한파 시 저온 환경에서 장시간 작업 중 저체온증·동상 등 한랭질환 발생 위험", "작업 전 기상청 한파 특보 확인", 2, 2, "IV", "1. 한파 특보(-15°C 이하) 발효 시 야외 노출 시간 최소화 및 방한복 지급\n2. 정기적인 휴식 시간 부여 및 실내 대피 장소 마련\n3. 작업자 이상 징후(오한, 손발 감각 저하) 발생 시 즉시 작업 중단"],
    ["비파괴검사", "방사선 작업종사자 건강검진 미실시 및 피폭선량 기록 누락으로 과피폭 위험", "방사선 작업종사자 건강검진 결과서 현장 비치 확인", 1, 3, "III", "1. 방사선 작업종사자 건강검진 결과서 현장 비치 및 주기적 갱신\n2. 개인선량계 패용 및 월별 피폭선량 기록·관리 철저\n3. ALARA 원칙 준수: 거리·차폐·시간 3원칙 적용"],
    ["비파괴검사", "서베이미터 미소지 또는 교정주기 초과 장비 사용으로 인한 방사선량 측정 오류 및 대피 지연", "작업 전 서베이미터 정상 작동(배터리) 및 교정 유효기간 점검", 1, 3, "III", "1. 현장 투입 전 서베이미터 작동 상태 일일 점검\n2. 검·교정 유효기간 초과 장비 절대 사용 금지\n3. 작업 중 상시 서베이미터 전원 ON 상태 유지"],
    ["비파괴검사", "방사선 투과 검사 진행 중 경고등(경광등) 미작동 및 감시 소홀로 인한 타 공정 근로자 통제구역 오진입", "경광등 작동 확인 및 타 공정 작업자 접근 통제", 2, 3, "V", "1. 선원 노출 시 경광등(회전 점멸등)이 즉각 작동되도록 사전 테스트\n2. 교차로 및 사각지대에 통제 인원 추가 배치\n3. 타 공정 근로자 대상 방사선 작업 시간 및 구역 사전 공지 전파"],
    ["비파괴검사", "콜리메이터(차폐체) 미사용 또는 파손품 사용으로 인한 방사선 과다 누출 및 주변 피폭", "콜리메이터 장착 상태 확인", 1, 3, "III", "1. 투과 검사 시 콜리메이터 장착 의무화 (미장착 작업 절대 금지)\n2. 작업 전 콜리메이터 파손 및 변형 여부 육안 점검\n3. 현장 여건상 통제 반경이 부족할 경우 추가 납 차폐막 설치"]
]
...
def init_data_file():
    ...
    cat_order = {"사전준비": 1, "반입 및 설치": 2, "이동": 3, "전기설비": 4, "비파괴검사": 5, "유지 및 운영": 6, "작업 후 정리": 7, "작업환경": 8}
    for d in [data_rt, data_ut, data_pt, data_container]:
        d.sort(key=lambda x: (cat_order.get(x[0], 99), get_risk_category(x[1])))
