...
CONFIG_FILE = "config_district_heating_safety_training.json"
DEFAULT_ATTENDEES = (
...
UNDERGROUND_MONTHLY_TOPICS = [
    ...
    "굴착기·인양장비와 지하 작업자 간 접촉방지·신호수 배치",
...
class SafetyTrainingApp:
    ...
    def _monthly_content_for(self, process, month):
        ...
        return (
            f"1. {month}월 공통 안전교육\n"
            f" - {common_topic}\n"
            " - 작업 전 TBM에서 작업순서, 위험요인, 근제대책과 작업중지 기준을 확인한다.\n"
            " - 작업구역 출입통제, 보호구 착용, 비상연락체계를 공유한다.\n\n"
            f"2. {process} 공정 중점교육\n"
            f" - {process_topic}\n"
            f"{process_core_safety}"
            " - 사용 장비와 계측기의 점검상태를 확인하고 불량품은 즉시 격리한다.\n"
            " - 작업 중 이상상태 발견 시 작업을 중지하고 관리감독자에게 보고한다.\n\n"
            "3. 지하 1.5~3m 배관 용접부 검사 공통위험\n"
            f" - {underground_topic}\n"
            " - 작업허가서와 당일 위험성평가를 확인하고 날씨·지반·장비 조건이 바뀌면 재평가한다.\n"
            " - 지하매설물의 도면과 현장 위치를 대조하고 미확인 매설물 발견 시 작업을 중지한다.\n"
            " - 열수송관의 차단·감압·배수와 잔류압력·잔류열을 확인하고 오조작 방지표지를 한다.\n"
            " - 진입 전과 재진입 전에 굴착면, 흑막이, 승강통로, 산소·유해가스 및 유수 여부를 확인한다.\n"
            " - 상부에 감시자를 배치하고 안전한 승강통로와 비상 탈출로를 확보한다.\n"
            ...
            " - 작업 종료 후 인원·장비·선원·화학물질 회수와 개구부 재차단을 확인한다."
        )
