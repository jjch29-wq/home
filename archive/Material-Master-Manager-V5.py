...
class MaterialManager:
            ...
            if not os.path.exists(self.db_path):
                ...
                try:
                    ...
                    if not self.daily_usage_df.empty:
                        ...
                        string_columns = ['Site', 'Note', 'User', '장비명', '검사방법']
    ...
    def create_draggable_container(self, parent, label_text, widget_class, config_key, manage_list_key=None, **widget_kwargs):
        ...
        # Store label widget and other metadata
        container._label_widget = lbl
        container._widget_class = widget_class
        container._widget_kwargs = widget_kwargs
        container._manage_list_key = manage_list_key  # Store what list this manages
        container._inner_widget = widget              # Store reference to the actual widget (Combobox/Entry)
    ...
    def refresh_ui_for_list_change(self, config_key):
        ...
        new_values = []
        if config_key == 'sites':
            new_values = sorted(self.sites)
        elif config_key == 'users':
            new_values = sorted(self.users)
        elif config_key == 'equipments':
            new_values = sorted(self.equipments)
            # Equipment also triggers material combo update separately
            self.update_material_combo()

        # Update all draggable widgets that track this list
        for key, container in self.draggable_items.items():
            if hasattr(container, '_manage_list_key') and container._manage_list_key == config_key:
                if hasattr(container, '_inner_widget') and hasattr(container._inner_widget, 'configure'):
                    # Check if it's a combobox or has 'values' attribute logic
                    try:
                        container._inner_widget['values'] = new_values
                    except:
                        pass
        
        # Also update standard non-draggable references if they exist (backward compatibility/specific cases)
        if config_key == 'sites':
            if hasattr(self, 'ent_daily_site'): self.ent_daily_site['values'] = new_values
            if hasattr(self, 'cb_trans_site'): self.cb_trans_site['values'] = new_values
            if hasattr(self, 'cb_daily_filter_site'):
                self.cb_daily_filter_site['values'] = ['전체'] + new_values
    ...
    def clone_widget(self, key):
        ...
        if hasattr(orig, '_widget_class'):
            ...
            manage_key = getattr(orig, '_manage_list_key', None)
            ...
            cont, w = self.create_draggable_container(
                ...
                orig._widget_class, 
                new_key, 
                manage_list_key=manage_key,  # Pass this so the clone also tracks list updates
                **orig._widget_kwargs
            )
    ...
    def reset_all_widgets_layout(self):
        """Reset all widgets to their original grid slots and unhide them"""
        if messagebox.askyesno("초기화", "모든 항목의 위치와 크기를 초기화하고 숨겨진 항목을 다시 표시하시겠습니까?"):
            # We use a copy of items because reset might modify things (though currently it doesn't modify draggable_items dict itself)
            for key, widget in list(self.draggable_items.items()):
                self.reset_widget_position(None, widget=widget)
            messagebox.showinfo("완료", "레이아웃이 초기화되었습니다.")
    ...
    def on_drag_start(self, event, widget=None):
        ...
        if self.layout_locked:
            return # Don't return "break" here to allow normal click selection possibly
    ...
    def setup_daily_usage_tab(self):
        ...
        self.btn_lock_layout = ttk.Button(header_frame, text="🔒 배치 고정", command=self.toggle_layout_lock, style="Lock.TButton")
        ...
        film_container.grid(row=5, column=0, padx=5, pady=5, sticky='w')
        ...
        columns = ('날짜', '현장', '작업자1', '장비명', '검사방법', '검사량', '필름매수', '단가', '출장비', '일식', '검사비', '품목명', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RT총계', '형광자분', '흑색자분', '백색페인트', '침투제', '세척제', '현상제', '형광침투제', '비고', '입력시간')
        ...
        # Column configuration
        col_widths = [100, 120, 100, 120, 100, 100, 100, 100, 100, 100, 100, 200, 70, 70, 70, 70, 70, 70, 70, 70, 80, 80, 80, 80, 80, 80, 80, 200, 150]
        # Columns to center-align (numeric values)
    ...
    def update_daily_usage_view(self):
        ...
        for idx, entry in filtered_df.iterrows():
            ...
            self.daily_usage_tree.insert('', tk.END, values=(
                ...
                mat_name,
                # film_count_str, # REMOVED
                *rtk_values,
                *ndt_values,
                note_val, # Use note_val directly since User is now a separate column
                entry_time
            ), tags=(str(idx),))
        ...
        if not filtered_df.empty:
            ...
            mandatory_cols = ['날짜', '현장', '작업자1', '장비명', '검사방법', '품목명', '비고', '입력시간']
            ...
            # If empty, show only mandatory columns or default set
            self.daily_usage_tree['displaycolumns'] = ['날짜', '현장', '장비명', '검사방법', '품목명', '비고', '입력시간']
        ...
        try:
            ...
            for idx in indices_to_delete:
                ...
                if not self.transactions_df.empty:
                    ...
                    ndt_materials = ["형광자분", "자분", "페인트", "침투제", "세척제", "현상제", "형광"]
                    ndt_mat_ids = []
    ...
    def export_all_daily_usage(self):
        ...
        try:
            ...
            if save_path:
                self.daily_usage_df.to_excel(save_path, index=False)
            ...
            for key, widget in self.draggable_items.items():
                if widget.winfo_manager() == 'place':
                    ...
                    if key.startswith('clone_'):
                        ...
                        config['draggable_geometries'][key]['widget_class_name'] = widget._widget_class.__name__
                        config['draggable_geometries'][key]['widget_kwargs'] = widget._widget_kwargs
                        # Save the list management key
                        if hasattr(widget, '_manage_list_key'):
                ...
                elif widget.winfo_manager() == '': # Hidden
    ...
    def load_tab_config(self):
        ...
        try:
            if os.path.exists(self.config_path):
                ...
                if hasattr(self, 'cb_warehouse'): self.cb_warehouse['values'] = self.warehouses
                
                # Restore equipments list
                ...
                if hasattr(self, 'cb_daily_equip'):
                    # Call update_material_combo to show combined equipment + material list correctly
                    self.update_material_combo()
                ...
                if config.get('selected_tab_id'):
                    self.notebook.select(config['selected_tab_id'])
                ...
                for key, geo in draggable_geos.items():
                    if key.startswith('memo_'):
                        ...
                        manage_key = geo.get('manage_list_key', None)
                        self.create_draggable_container(self.entry_inner_frame, label, cls, key, manage_list_key=manage_key, **kwargs)
