    ...
    def load_data(self):
        try:
            ...
            if not os.path.exists(self.db_path):
                ...
                try:
                    self.monthly_usage_df = pd.read_excel(self.db_path, sheet_name='MonthlyUsage', dtype={'Site': str, 'Note': str, 'Material ID': int})
                ...
                try:
                    ...
                    if not self.daily_usage_df.empty:
                        ...
                        # Fill NaNs in string columns with empty string
                        string_columns = ['Site', 'Material ID', 'Note', 'User']
                        for col in string_columns:
                            if col in self.daily_usage_df.columns:
                                self.daily_usage_df[col] = self.daily_usage_df[col].fillna('')
                    ...
                    self.daily_usage_df = pd.DataFrame(columns=['Date', 'Site', 'Material ID', 'Usage', 'Note', 'Entry Time', 
                                                        'RTK_센터미스', 'RTK_농도', 'RTK_마킹미스', 'RTK_필름마크', 
                                                        'RTK_취급부주의', 'RTK_고객불만', 'RTK_기타', 'User'])
    ...
    def add_daily_usage_entry(self):
        ...
        if film_count > 0:
            new_transaction = {
                ...
                'Quantity': film_count,
                'User': '현장사용',
                'Note': f'{site} 현장 사용 (필름매수 차감)'
            }
            self.transactions_df = pd.concat([self.transactions_df, pd.DataFrame([new_transaction])], ignore_index=True)
            stock_info += f"\n• {pure_mat_name}: {film_count:.1f} 차감"

        # 2. Deduct NDT materials
        for ndt_name, qty in ndt_usage.items():
            # Find NDT material in database - check both 품목명 and 모델명
            ndt_mat_rows = self.materials_df[
                (self.materials_df['품목명'] == ndt_name) | 
                (self.materials_df['모델명'] == ndt_name)
            ]
            
            if not ndt_mat_rows.empty:
                ndt_mat_id = ndt_mat_rows['Material ID'].values[0]
                actual_mat_name = ndt_mat_rows['품목명'].values[0]
                ndt_transaction = {
                    'Date': usage_date,
