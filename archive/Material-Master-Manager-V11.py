...
class SuggestionWindow:
    ...
    def show(self, filtered_values, extra_px=0):
        ...
        width = self.widget.winfo_width() + extra_px
        item_height = 24
        max_items = 12
...
class WorkerCompositeWidget(ttk.Frame):
    ...
    def __init__(self, parent, shift_width=5, enable_autocomplete=False, user_list=None, **kwargs):
        ...
        if enable_autocomplete and user_list is not None:
            ...
            self._suggestion_win = SuggestionWindow(self.cb_name)

            def perform_filter():
                typed = self.cb_name.get()
                if not typed:
                    self.cb_name['values'] = user_list
                    self._suggestion_win.hide()
                    return
                
                # Use live values from the widget to ensure sync with global list
                all_vals = self.cb_name['values']
                ...
                if filtered:
                    try:
                        ...
                        max_w = max(get_visual_width(v) for v in filtered)
                        threshold = 8 
                        if max_w > threshold:
                            extra_px = (max_w - threshold) * 12 + 30
                    except: pass
                
                # Show custom window instead of native dropdown
                self._suggestion_win.show(filtered, extra_px)

            def on_keyrelease(event):
                # Handle navigation keys for custom window
                if event.keysym == "Down" and self._suggestion_win.active:
                    self._suggestion_win.move_selection(1)
                    return "break"
                if event.keysym == "Up" and self._suggestion_win.active:
                    self._suggestion_win.move_selection(-1)
                    return "break"
                if event.keysym == "Return" and self._suggestion_win.active:
                    if self._suggestion_win.confirm_selection():
                        return "break"
                
                if event.keysym in ("Left", "Right", "Escape", "Tab"): # Removed Up, Down, Return
                    return
                
                if self._autocomplete_timer:
                    self.after_cancel(self._autocomplete_timer)
                self._autocomplete_timer = self.after(50, perform_filter)
            
            self.cb_name.bind('<KeyRelease>', on_keyrelease)
            
            def handle_extra_nav(event):
                if not self._suggestion_win.active: return
                if event.keysym == "Down": self._suggestion_win.move_selection(1); return "break"
                if event.keysym == "Up": self._suggestion_win.move_selection(-1); return "break"
                if event.keysym == "Return": 
                    if self._suggestion_win.confirm_selection(): return "break"
                    
            self.cb_name.bind('<KeyPress-Down>', handle_extra_nav)
            self.cb_name.bind('<KeyPress-Up>', handle_extra_nav)
            self.cb_name.bind('<KeyPress-Return>', handle_extra_nav)

            def on_focus_out(event):
                # Small delay to allow clicking on listbox before hiding
                self.after(200, self._suggestion_win.hide)
            # Use add=True to avoid overwriting external save bindings
            self.cb_name.bind('<FocusOut>', on_focus_out, add=True)
...
class MaterialManager:
    ...
    def enable_autocomplete(self, combobox, values_list_attr=None, values_list=None):
        ...
        def perform_filter(event_widget):
            ...
            # Show custom window
            suggest_win.show(filtered, extra_px)
        ...
        def on_keyrelease(event):
            ...
            if event.keysym == "Up" and suggest_win.active:
                suggest_win.move_selection(-1)
                return "break"
            if event.keysym == "Return" and suggest_win.active:
                if suggest_win.confirm_selection():
                    return "break"
        ...
        def on_focus_in(event):
            ...
            if not typed:
                if values_list_attr:
                    event.widget['values'] = getattr(self, values_list_attr, [])
                elif values_list is not None:
                    event.widget['values'] = values_list
        ...
        for _, entry in grouped.iterrows():
            ...
            for col in worker_cols:
                if col in entry and entry[col]:
                    all_workers.update([w.strip() for w in entry[col].split(',') if w.strip()])
            worker_str = ", ".join(sorted(all_workers))
    ...
    def setup_monthly_usage_tab(self):
        ...
        for col in site_cols:
            ...
            self.site_summary_tree.column(col, width=80, anchor='center')
        ...
        for col in worker_cols:
            ...
            self.worker_summary_tree.column(col, width=width, anchor='center')
    ...
    def update_monthly_usage_view(self):
        ...
        if not grouped.empty:
            ...
            if worker_data:
                ...
                worker_df = worker_df[worker_df['WorkerName'].notna() & (worker_df['WorkerName'].str.strip() != '') & (worker_df['WorkerName'].str.strip() != 'nan')]
                
                if not worker_df.empty:
                    worker_df['OT_H'] = worker_df['OTValue'].apply(self._parse_ot_hours)
                    # Split hours into Day (18-22) and Night (22+) windows
                    worker_df[['OT_H_Day', 'OT_H_Night']] = worker_df['OTValue'].apply(
                        lambda x: pd.Series(self._calculate_split_ot_hours(x))
                    )
                    worker_df['OT_A'] = worker_df['OTValue'].apply(self.calculate_ot_amount)
                    worker_df['Count'] = 1.0
                    
                    # Create binary indicators for shift types
                    ...
                    # Split logic for the requested columns:
                    # Hours: '주간', '야간', '휴일'
                    # Amounts: '주간OT', '야간OT', '휴일OT'
                    
                    # Holiday takes precedence: if it's a holiday shift, all OT goes to '휴일' bucket
                    ...
                    # Non-holiday shifts use the window-based split
                    ...
                    # Calculate amounts for Day/Night windows in non-holiday shifts
                    # (Simplified: Day amount = hours * 4000, Night amount = hours * 5000)
                    ...
                    for _, row in worker_summary.iterrows():
                        self.worker_summary_tree.insert('', tk.END, values=(
                            row['WorkerName'],
                            f"{row['Count']:.1f}",
                            f"{row['H_Day']:.1f}",      # 주간 (Hours)
                            f"{row['H_Night']:.1f}",    # 야간 (Hours)
                            f"{row['H_Holiday']:.1f}",  # 휴일 (Hours)
                            f"{row['OT_H']:.1f}",        # OT시간 (Total Hours)
                            f"{row['A_Day']:,.0f}",      # 주간OT (Amount)
                            f"{row['A_Night']:,.0f}",    # 야간OT (Amount)
                            f"{row['A_Holiday']:,.0f}",  # 휴일OT (Amount)
                            f"{row['OT_A']:,.0f}"        # OT합계 (Total Amount)
                        ))

            # Re-apply stretch=False and minwidth
            ...
            if hasattr(self, 'tab_config') and 'monthly_usage_col_widths' in self.tab_config:
                ...
                for col in self.monthly_usage_tree['columns']:
                    if col in saved_widths:
                        try:
                            self.monthly_usage_tree.column(col, width=int(saved_widths[col]), stretch=False)
                        except: pass
    ...
    def setup_daily_usage_tab(self):
        ...
        date_container, self.ent_daily_date = self.create_draggable_container(
            ...
            width=10, date_pattern='yyyy-mm-dd', locale='ko_KR',
        ...
        amount_container, self_ent_daily_test_amount = self.create_draggable_container(
            self.left_pane, "량:", ttk.Entry, 'amount_box_geometry', width=6,
            grid_info={'row': 2, 'column': 1, 'padx': 1, 'pady': 1, 'sticky': 'w'}
        )
        # Removed default "0"
        self.ent_daily_test_amount = self_ent_daily_test_amount
        u_price_container, self_ent_daily_unit_price = self.create_draggable_container(
            self.left_pane, "단가:", ttk.Entry, 'u_price_box_geometry', width=8,
            grid_info={'row': 2, 'column': 2, 'padx': 1, 'pady': 1, 'sticky': 'w'}
        )
        # Removed default "0"
        self.ent_daily_unit_price = self_ent_daily_unit_price
        travel_container, self_ent_daily_travel_cost = self.create_draggable_container(
            self.left_pane, "출장:", ttk.Entry, 'travel_box_geometry', width=8,
            grid_info={'row': 2, 'column': 3, 'padx': 1, 'pady': 1, 'sticky': 'w'}
        )
        # Removed default "0"
        self.ent_daily_travel_cost = self_ent_daily_travel_cost
        meal_container, self_ent_daily_meal_cost = self.create_draggable_container(
            self.left_pane, "일식:", ttk.Entry, 'meal_box_geometry', width=8,
            grid_info={'row': 2, 'column': 4, 'padx': 1, 'pady': 1, 'sticky': 'w'}
        )
        # Removed default "0"
        self.ent_daily_meal_cost = self_ent_daily_meal_cost
        fee_container, self_ent_daily_test_fee = self.create_draggable_container(
            self.left_pane, "검사비:", ttk.Entry, 'fee_box_geometry', width=8,
            grid_info={'row': 2, 'column': 5, 'padx': 1, 'pady': 1, 'sticky': 'w'}
        )
        # Removed default "0"
        self.ent_daily_test_fee = self_ent_daily_test_fee
        film_container, self_ent_film_count = self.create_draggable_container(
            self.left_pane, "필름:", ttk.Entry, 'film_box_geometry', width=4,
            grid_info={'row': 2, 'column': 6, 'padx': 1, 'pady': 1, 'sticky': 'w'}
        )
        # Removed default "0"
        self.ent_film_count = self_ent_film_count
    ...
    def update_daily_usage_view(self):
        ...
        if hasattr(self, 'cb_daily_filter_material'):
            # Check if MaterialID column exists
            if 'MaterialID' in self.daily_usage_df.columns:
                ...
                # If MaterialID doesn't exist, set empty filter
        ...
        if hasattr(self, 'cb_daily_filter_worker'):
            ...
            # Filter out any worker name that exactly matches these keywords (after removing all spaces and normalizing parentheses)
            ...
            def is_valid_worker_name(name):
                ...
                # Normalize: remove spaces, convert all bracket types to standard ()
                norm = s.replace(' ', '').replace('（', '(').replace('）', ')').replace('[', '(').replace(']', ')')
                
                # Check 1: Exact matches against bad keywords (including common variations)
                bad_keywords = ['휴일', '주간', '야간', '공휴일', '휴무', '주야간', '주간근무', '야간근무', '휴일근무', 
                               '잔업', '특근', '철야', '월차', '연차', '반차']
                if norm in bad_keywords:
                    return False
                
                # Check 2: Parenthesized matches (e.g., (휴일), (주간))
                # If it looks like a parenthesized term, check if it contains any bad keyword
                if norm.startswith('(') and norm.endswith(')'):
                    inner = norm[1:-1]
                    # If the inner part contains a bad keyword, exclude it
                    if any(k in inner for k in ['휴일', '주간', '야간', '공휴일', '휴무']):
                        return False
                        
                return True

            unique_workers = ['전체'] + sorted([w for w in all_workers if is_valid_worker_name(w)])
            print(f"DEBUG: Unique workers list generated (Count: {len(unique_workers)}): {unique_workers}")
        ...
        for idx, entry in filtered_df.iterrows():
            ...
            if filter_worker != '전체':
                ...
                for i in range(1, 11):
                    ...
                    u_val = clean_str(entry.get(u_key, ''))
                    if u_val == filter_worker:
            ...
            for i in range(1, 11):
                ...
                if ot_str:
                    try:
                        ...
                        # [BUGFIX] Indiv totals should accumulate regardless of display logic, 
                        # but we want totals to reflect Filtered View?
                        # Actually total_indiv_ot_hours is mostly for full view column sums.
                        # If we filter, maybe we shouldn't sum other cols?
                        # But row_ots has holes, so effectively we are only summing valid ones?
                        # No, total_indiv_ot_hours[i-1] += ...
    ...
    def calculate_ot_amount(self, ot_value):
        ...
        try:
            ...
            # Extract start hour for fine-grained calculation if range provided
            start_hour = 18
            if ':' in val and any(x in val for x in ['-', '~']):
                sep = '-' if '-' in val else '~'
                try:
                    start_time = val.split(sep)[0]
            ...
            if ':' in val and any(x in val for x in ['-', '~']):
                ...
                while remaining_hours > 0 and current_time < 36: # Allow overnight up to 12pm next day
                    if 18 <= current_time < 22: rate = 4000
                    elif current_time >= 22: rate = 5000
                    else: rate = 3000
                    
                    next_change = 22 if current_time < 22 else (start_hour + hours)
                    h_in_period = min(remaining_hours, next_change - current_time)
                    if h_in_period <= 0: break
                    
                    amount += h_in_period * rate
                    remaining_hours -= h_in_period
                    current_time += h_in_period
            else:
                # Simple duration
    ...
    def _parse_ot_hours(self, ot_value):
        ...
        try:
            ...
            # 1. Check for time range "18:00~22:00"
            ...
            # 2. Check for simple ':' format "2:30"
            colon_match = re.search(r'^(\d{1,2}):(\d{1,2})$', val)
            if colon_match:
                h, m = map(int, colon_match.groups())
                return h + (m / 60)

            # 3. Check for "N시간" or "N.N시간" or "N시"
            dur_match = re.search(r'(\d+\.?\d*)\s*(시간|시|hr|h)', val)
            if dur_match:
                return float(dur_match.group(1))

            # 4. Fallback to just extracting the first float/int found
        ...
        try:
            ...
            if os.path.exists(self.config_path):
                ...
                def delayed_restore():
                    ...
                    self.root.after(2000, finalize_loading) 
