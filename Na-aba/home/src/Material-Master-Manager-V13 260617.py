    ...
    def setup_daily_usage_tab(self):
        ...
        def _calculate_ndt_fee():
            try:
                ...
                # 상수
                mat_cost = {'RT (B필름: 3⅓"x17")': 3379, 'RT (A필름: 3⅓"x12")': 2540, 'RT (A/2필름: 3⅓"x6")': 1515, 'UT': 1115, 'PT': 3971}
                lab_cost = {
                ...
                if ndt_type == "RT":
                    if "b" in mat_type or "17" in mat_type:
                        ...
                        mat_unit_cost = 3379 # Default to B film
                elif ndt_type == "UT":
                    mat_unit_cost = 1115
                elif ndt_type == "PT":
                    mat_unit_cost = 3971
                ...
                lab_unit = lab_cost.get(work_time, lab_cost["일반"]).get(ndt_type, 0)
                total_lab = int(adj_qty * lab_unit)
                
                overhead = int(total_lab * (self.ndt_overhead_var.get()/100))
                tech = int((total_lab + overhead) * (self.ndt_tech_var.get()/100))
