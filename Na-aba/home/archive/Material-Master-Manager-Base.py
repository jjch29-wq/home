...
class MaterialManager:
    def __init__(self, root):
        ...
        self.config_path = r'c:\Users\jjch2\Desktop\보고서\Project PROVIDENCE\Request\PMI\Material_Manager_Config.json'
    ...
    def setup_stock_tab(self):
        ...
        # Refresh Button
        btn_refresh = ttk.Button(first_row, text="재고 새로고침", command=self.update_stock_view)
        btn_refresh.pack(side='left', padx=5)
        
        # Export Button - moved to first position for visibility
        btn_export = ttk.Button(first_row, text="📊 엑셀 내보내기", command=self.export_stock_to_excel, width=15)
        ...
        btn_alert = ttk.Button(second_row, text="재주문 필요 항목 보기", command=self.show_low_stock)
        btn_alert.pack(side='left', padx=5)
        
        # Delete Button
        btn_delete = ttk.Button(second_row, text="품목 삭제", command=self.delete_selected_material)
        btn_delete.pack(side='left', padx=5)
        
        # Edit Button
        btn_edit = ttk.Button(second_row, text="품목 수정", command=self.open_edit_material_dialog)
        btn_edit.pack(side='left', padx=5)
        
        # Select All Button
        btn_select_all = ttk.Button(second_row, text="전체 선택", command=self.select_all_stock)
    ...
    def setup_inout_tab(self):
        ...
        self.cb_material = ttk.Combobox(trans_frame, state="readonly", width=65, font=('Arial', 8))
        ...
        ttk.Label(trans_frame, text="비고:").grid(row=2, column=0, padx=5, pady=2, sticky='w')
        self.ent_note = ttk.Entry(trans_frame, width=30)
        self.ent_note.grid(row=2, column=1, padx=5, pady=2, columnspan=3)
    ...
    def update_material_combo(self):
        ...
        if not self.materials_df.empty:
            # Create list with format "품목명 (SN: SN번호) - 규격" or variations
            for _, mat in self.materials_df.iterrows():
                name = mat['품목명']
                sn = mat.get('SN', '')
                spec = mat.get('규격', '')
    ...
    def setup_report_tab(self):
        ...
        columns = ('연도', '월', '현장', '품목명', '필름매수', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RT총계', '자분', '페인트', '침투제', '세척제', '현상제', '비고')
        ...
        col_widths = [80, 60, 120, 200, 100, 70, 70, 70, 70, 70, 70, 70, 70, 80, 80, 80, 80, 80, 200]
        # Columns to center-align (numeric values)
        center_cols = ['필름매수', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RT총계', '자분', '페인트', '침투제', '세척제', '현상제']
    ...
    def update_report_view(self):
        ...
        # Add NDT materials
        ndt_materials = ['NDT_자분', 'NDT_페인트', 'NDT_침투제', 'NDT_세척제', 'NDT_현상제']
    ...
    def export_stock_to_excel(self):
        ...
        try:
            ...
            if save_path:
                ...
                stock_df.to_excel(save_path, index=False)
    ...
    def setup_daily_usage_tab(self):
        ...
        self.ent_daily_date = ttk.Entry(entry_frame, width=30)
        self.ent_daily_date.grid(row=0, column=1, padx=5, pady=5)
        # Set default to today
        today = datetime.datetime.now().strftime('%Y-%m-%d')
        self.ent_daily_date.insert(0, today)
        ttk.Label(entry_frame, text="(형식: YYYY-MM-DD)", font=('Arial', 8)).grid(row=0, column=2, padx=5, pady=5, sticky='w')
        ...
        self.ent_daily_start_date = ttk.Entry(filter_frame, width=12)
        self.ent_daily_start_date.pack(side='left', padx=5)
        # Default to 7 days ago
        start_date = (datetime.datetime.now() - datetime.timedelta(days=7)).strftime('%Y-%m-%d')
        self.ent_daily_start_date.insert(0, start_date)
        
        ttk.Label(filter_frame, text="종료일:").pack(side='left', padx=5)
        self.ent_daily_end_date = ttk.Entry(filter_frame, width=12)
        self.ent_daily_end_date.pack(side='left', padx=5)
        self.ent_daily_end_date.insert(0, today)
        
        ttk.Label(filter_frame, text="현장:").pack(side='left', padx=5)
        self.ent_daily_filter_site = ttk.Entry(filter_frame, width=15)
        self.ent_daily_filter_site.pack(side='left', padx=5)
        ...
        columns = ('날짜', '현장', '품목명', '필름매수', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RT총계', '자분', '페인트', '침투제', '세척제', '현상제', '비고', '입력시간')
        ...
        # Column configuration
        col_widths = [100, 120, 200, 100, 70, 70, 70, 70, 70, 70, 70, 70, 80, 80, 80, 80, 80, 200, 150]
        # Columns to center-align (numeric values)
        center_cols = ['필름매수', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RT총계', '자분', '페인트', '침투제', '세척제', '현상제']
    ...
    def add_daily_usage_entry(self):
        ...
        self.ent_daily_date.delete(0, tk.END)
        self.ent_daily_date.insert(0, datetime.datetime.now().strftime('%Y-%m-%d'))
    ...
    def update_daily_usage_view(self):
        ...
        filter_site = self.ent_daily_filter_site.get().strip()
        ...
        if filter_site:
            filtered_df = filtered_df[filtered_df['Site'].str.contains(filter_site, case=False, na=False)]
        ...
        for idx, entry in filtered_df.iterrows():
            ...
            if not mat_row.empty:
                mat_name = mat_row.iloc[0]['품목명']  # Using 재고현황's 품목명 field
            else:
                mat_name = f"ID: {mat_id}"
            ...
            if pd.notna(film_count):
                film_count_str = f"{film_count:.1f}"
            else:
                film_count_str = "0.0"
            ...
            for category in rtk_categories:
                ...
                if pd.notna(value):
                    rtk_values.append(f"{value:.1f}")
                    total += float(value)
                else:
                    rtk_values.append("0.0")
            
            rtk_values.append(f"{total:.1f}")  # Add total
