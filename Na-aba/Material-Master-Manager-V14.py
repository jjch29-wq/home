...
class MaterialManager:
    def __init__(self, root):
        ...
        self.test_items = [] # [NEW] Inspection items list (검사품명)
        self.applied_codes = [] # [NEW] Applied codes list (적용코드)
        ...
        self.CORE_DRAGGABLE_KEYS = [
            ...
            'inspection_details_box_geometry'
        ]
    ...
    def load_data(self):
        ...
        try:
            ...
            self.daily_usage_df = pd.DataFrame(columns=['Date', 'Site', 'MaterialID', 'Usage', 'Note', 'EntryTime', 
                                                'RTK_센터미스', 'RTK_농도', 'RTK_마킹미스', 'RTK_필름마크', 
                                                'RTK_취급부주의', 'RTK_고객불만', 'RTK_기타', '장비명', '검사품명', '적용코드', '검사방법', '검사량',
                                            '차량번호', '주행거리', '차량점검', '차량비고'])
    ...
    def refresh_ui_for_list_change(self, config_key):
        ...
        list_map = {
            ...
            'applied_codes': self.applied_codes
        }
        ...
        elif config_key == 'test_items':
            if hasattr(self, 'cb_daily_test_item'): self.cb_daily_test_item['values'] = sorted_vals
        elif config_key == 'applied_codes':
            if hasattr(self, 'cb_daily_applied_code'): self.cb_daily_applied_code['values'] = sorted_vals
    ...
    def setup_daily_usage_tab(self):
        ...
        def on_mat_select(e):
            self.root.after(10, self.cb_daily_test_item.focus_set)
            return "break"
        ...
        # --- Inspection Details Box (NEW) ---
        insp_box, insp_inner = self.create_draggable_container(
            self.entry_inner_frame, "검사 상세 정보", ttk.Frame, 'inspection_details_box_geometry'
        )
        
        # Grid inside Inspection Details
        ttk.Label(insp_inner, text="검사품명:").grid(row=0, column=0, padx=2, pady=1, sticky='e')
        self.cb_daily_test_item = ttk.Combobox(insp_inner, width=20, values=self.test_items)
        self.cb_daily_test_item.grid(row=0, column=1, padx=2, pady=1, sticky='w')
        
        btn_item_mgr = ttk.Label(insp_inner, text="⚙", font=('Arial', 9), cursor='hand2')
        btn_item_mgr.grid(row=0, column=2, padx=1)
        btn_item_mgr.bind('<Button-1>', lambda e: self.open_list_management_dialog('test_items'))
        
        ttk.Label(insp_inner, text="적용코드:").grid(row=1, column=0, padx=2, pady=1, sticky='e')
        self.cb_daily_applied_code = ttk.Combobox(insp_inner, width=20, values=self.applied_codes)
        self.cb_daily_applied_code.grid(row=1, column=1, padx=2, pady=1, sticky='w')
        
        btn_code_mgr = ttk.Label(insp_inner, text="⚙", font=('Arial', 9), cursor='hand2')
        btn_code_mgr.grid(row=1, column=2, padx=1)
        btn_code_mgr.bind('<Button-1>', lambda e: self.open_list_management_dialog('applied_codes'))

        def on_insp_item_select(e):
            self.root.after(10, self.cb_daily_applied_code.focus_set)
            return "break"
        self.cb_daily_test_item.bind('<<ComboboxSelected>>', on_insp_item_select)
        self.cb_daily_test_item.bind('<Return>', on_insp_item_select)

        def on_applied_code_select(e):
            self.root.after(10, self.cb_daily_test_method.focus_set)
            return "break"
        self.cb_daily_applied_code.bind('<<ComboboxSelected>>', on_applied_code_select)
        self.cb_daily_applied_code.bind('<Return>', on_applied_code_select)
        
        insp_box.grid(row=3, column=0, padx=1, pady=1, sticky='nw')
        ...
        try:
            ...
            self.entry_inner_frame.grid_rowconfigure(3, minsize=100) # Increased minsize
            self.entry_inner_frame.grid_rowconfigure(4, minsize=60)
        except Exception:
            pass
        ...
        save_btn_container.grid(row=4, column=0, padx=1, pady=1, sticky='nw')
        ...
        columns = ('날짜', '현장', '작업자', '작업시간', 'OT1', 'OT2', 'OT3', 'OT4', 'OT5', 'OT6', 'OT7', 'OT8', 'OT9', 'OT10', '장비명', '검사품명', '적용코드', '검사방법', '수량', '필름매수', '단가', '출장비', '일식', '검사비', 'OT시간', 'OT금액', '품목명', '센터미스', '농도', '마킹미스', '필름마크', '취급부주의', '고객불만', '기타', 'RTK총계', '형광자분', '흑색자분', '백색페인트', '침투제', '세척제', '현상제', '형광침투제', '비고', '입력시간', '차량번호', '주행거리', '차량점검', '차량비고', '(Full작업자)')
        ...
        col_widths = {
            ...
            '장비명': 130, '검사품명': 150, '적용코드': 120, '검사방법': 90, 
    ...
    def add_daily_usage_entry(self):
        ...
        try:
            ...
            test_item = self.cb_daily_test_item.get().strip() if hasattr(self, 'cb_daily_test_item') else ''
            applied_code = self.cb_daily_applied_code.get().strip() if hasattr(self, 'cb_daily_applied_code') else ''
    ...
    def open_edit_daily_usage_dialog(self):
        ...
        cb_method = ttk.Combobox(basic_frame, width=10, values=['RT', 'PAUT', 'UT', 'MT', 'PT', 'PMI'])
        ...
        # [NEW] Inspection Item & Applied Code
        ttk.Label(basic_frame, text="검사품명:").grid(row=3, column=0, padx=5, pady=5, sticky='w')
        cb_test_item = ttk.Combobox(basic_frame, width=20, values=self.test_items)
        cb_test_item.set(self.clean_nan(entry_data.get('검사품명', '')))
        cb_test_item.grid(row=3, column=1, padx=5, pady=5, sticky='w')
        fields['검사품명'] = cb_test_item

        ttk.Label(basic_frame, text="적용코드:").grid(row=3, column=2, padx=5, pady=5, sticky='w')
        cb_applied_code = ttk.Combobox(basic_frame, width=20, values=self.applied_codes)
        cb_applied_code.set(self.clean_nan(entry_data.get('적용코드', '')))
        cb_applied_code.grid(row=3, column=3, padx=5, pady=5, sticky='w')
        fields['적용코드'] = cb_applied_code
        ...
        self.enable_autocomplete(cb_test_item, values_list_attr='test_items')
        self.enable_autocomplete(cb_applied_code, values_list_attr='applied_codes')
        ...
        def save_edits():
            ...
            new_data['검사품명'] = cb_test_item.get().strip()
            new_data['적용코드'] = cb_applied_code.get().strip()
