...
GLOSSARY.append({
    "term": "결정립 크기",
    "aliases": ["재질의 결정립 크기", "재질의 결정립크기", "결정립크기", "결정립", "grain size", "입도"],
    "category": "UT 재료",
    "definition": "금속재료를 구성하는 개별 결정립의 평균적인 크기이다. 제조법, 열처리 및 용접 열이력에 따라 달라진다.",
    "relation": "결정립이 작으면 초음파 산란이 적어 높은 주파수를 사용할 수 있다. 결정립이 크거나 초음파 파장과 비슷해지면 결정립 경계에서 산란과 구조 잡음이 증가하여 감쇠가 커지고 투과력이 감소한다. 주조품, 오스테나이트계 스테인리스강 및 용접금속과 같은 조대결정 재료에는 일반적으로 낮은 주파수를 사용해 파장을 길게 한다. 다만 저주파는 작은 결함 감도와 분해능이 낮으므로 동종 재질 대비시험편으로 검사 성능을 확인해야 한다.",
    "formula": "미세 결정립 → 산란·감쇠 감소 → 고주파 적용 가능\n조대 결정립 또는 결정립 크기 ≈ 파장 → 산란·구조 잡음 증가 → 저주파 적용\n주파수 저하 → 투과력 증가, 작은 결함 감도·분해능 감소",
    "related": ["주파수", "파장", "산란", "감쇠", "분해능", "대비시험편"],
    "questions": ["탐촉자와 주파수 선정", "초음파 탐상에서 결정립 크기가 미치는 영향"],
})

GLOSSARY.append({
    "term": "중심주파수",
    "aliases": ["중심 주파수", "center frequency", "centre frequency", "피크 주파수", "fc"],
    "category": "UT 장비",
    "definition": "탐촉자가 송신하거나 수신하는 주파수 대역을 대표하는 주파수로, 일반적으로 주파수 스펙트럼에서 진폭이 가장 큰 피크 주파수를 말한다.",
    "relation": "표시값이 5 MHz인 탐촉자도 5 MHz의 단일 성분만 발생하는 것이 아니라 일정한 주파수 대역을 갖는다. 중심주파수가 높으면 파장이 짧아 작은 결함 감도와 분해능에 유리하지만 감쇠와 산란이 증가한다. 중심주파수가 낮으면 두꺼운 재료와 조대결정 재료의 투과에 유리하지만 작은 결함 감도는 낮아진다. 표시 주파수와 실제 중심주파수는 다를 수 있으므로 주파수 스펙트럼으로 확인하고 대역폭과 함께 평가한다.",
    "formula": "λ = c/fc\nλ: 파장, c: 재료의 음속, fc: 중심주파수\n\n중심주파수: 주파수 대역의 대표 위치\n대역폭: 주파수 성분이 분포하는 범위",
    "related": ["주파수", "파장", "대역폭", "탐촉자", "분해능", "감쇠", "산란"],
    "questions": ["탐촉자와 주파수 선정", "탐촉자의 중심주파수와 대역폭"],
})

GLOSSARY.append({
    "term": "배열소자",
    "aliases": ["배열 소자", "array element", "array elements", "PAUT 소자", "압전소자"],
    "category": "고급 UT",
    "definition": "PAUT 탐촉자 내부에 일정한 간격으로 배열되어 전기신호와 초음파를 상호 변환하는 개별 압전소자이다.",
    "relation": "장비가 각 배열소자의 송·수신 시점에 지연법칙을 적용하면 초음파 빔을 전자적으로 조향하거나 특정 깊이에 집속할 수 있다. 전체 소자수는 탐촉자에 내장된 소자의 수이고, 활성소자수는 한 번의 빔 형성에 사용하는 소자의 수이다. 활성소자군의 위치를 순차적으로 이동하면 탐촉자를 움직이지 않고 선형주사할 수 있다. 소자수가 많고 피치가 작으면 조향과 집속을 정밀하게 제어할 수 있지만 채널 수와 데이터 처리량이 증가하며, 피치가 지나치게 크면 그레이팅 로브가 발생할 수 있다.",
    "formula": "피치(p): 인접 소자 중심 사이의 거리\n소자 간격(g): 두 소자 사이의 빈 공간\n소자 폭(e): p = e + g\n개구(aperture): 동시에 사용하는 활성소자군의 전체 크기\n지연법칙(delay law): 소자별 송·수신 시간차 → 빔 조향·집속",
    "related": ["PAUT", "탐촉자", "활성소자수", "피치", "개구", "지연법칙", "그레이팅 로브", "빔 조향", "집속"],
    "questions": ["PAUT의 원리와 특성", "PAUT 검사설계", "배열소자의 피치와 그레이팅 로브"],
})
