...
class MaterialManager:
    ...
    def create_draggable_container(self, parent, label_text, widget_class, config_key, manage_list_key=None, **widget_kwargs):
        ...
        # Store label widget for renaming
    ...
    def refresh_ui_for_list_change(self, config_key):
        ...
        if config_key == 'sites':
            if hasattr(self, 'ent_daily_site'): self.ent_daily_site['values'] = sorted(self.sites)
            if hasattr(self, 'cb_trans_site'): self.cb_trans_site['values'] = sorted(self.sites)
            if hasattr(self, 'cb_daily_filter_site'):
                self.cb_daily_filter_site['values'] = ['전체'] + sorted(self.sites)
        elif config_key == 'users':
            if hasattr(self, 'cb_daily_user'): self.cb_daily_user['values'] = sorted(self.users)
        elif config_key == 'equipments':
            self.update_material_combo() # Equipment impacts material combo in this logic
            ...
            if os.path.exists(self.config_path):
                ...
                for key, geo in draggable_geos.items():
                    if key.startswith('memo_'):
                        ...
                        self.create_draggable_container(self.entry_inner_frame, label, cls, key, **kwargs)
